...
class PDF extends FPDF
	{
		function Header()
		{
			...
			$this->SetXY(90,11);
			$this->Cell(0,5,'GERENCIA REGIONAL DE TRIBUTOS INTERNOS - '.mb_convert_case(utf8_decode($region), MB_CASE_UPPER, "ISO-8859-1"),0,1,'C');
			$this->SetXY(90,16);
			...
			$this->SetXY(90,21);
			$this->Cell(0,5,mb_convert_case(utf8_decode($area).' de '.$texto1, MB_CASE_UPPER, "ISO-8859-1"),0,1,'C');
			$this->Ln(7);
			
			//---------- FECHA DE LA TRANSFERENCIA
			$consulta_x = "SELECT fecha_transferencia_not as fecha FROM vista_liquidacion_planillas WHERE memo = ".$_GET['memo']." AND sector=".$_GET['sede'].";";
			$tabla_x = mysql_query($consulta_x);
			$regstro_x = mysql_fetch_object($tabla_x);
			...
			$this->Cell(0,5,'Relación de Expedientes Exportados a Notificación bajo la Transferencia N°: '.$_GET['memo'].' el día '.voltea_fecha($fecha),0,1,'C');
			$this->Ln(4);
		...
		$consulta = "SELECT COUNT(DISTINCTROW sector, origen_liquidacion, anno_expediente, num_expediente) AS cantidad FROM vista_liquidacion_planillas WHERE memo = ".$_GET['memo']." AND sector=".$_GET['sede'].";";
		$tabla = mysql_query($consulta);
		if ($providencia = mysql_fetch_object($tabla)) { $cant_prov = $providencia->cantidad; };

		//OBTENEMOS LA CANTIDAD DE PLANILLAS
		$consulta = "SELECT COUNT(num_expediente) AS cantidad FROM vista_liquidacion_planillas WHERE memo = ".$_GET['memo']." AND sector=".$_GET['sede'].";";
		$tabla = mysql_query($consulta);
		...
		$consulta = "SELECT usuario_transferencia_a_not FROM liquidacion WHERE memo = ".$_GET['memo']." AND sector=".$_GET['sede'].";";
		$tabla = mysql_query($consulta);
		$registro = mysql_fetch_object($tabla);
		//---------------
		list ($nombre_usuario) = funcion_funcionario($registro->usuario_transferencia_a_not);
		//---------------
		$this->SetY(-25);
...
$consulta = "SELECT rif, contribuyente, liquidacion, periodoinicio, periodofinal, fecha_liquidacion, siglas, (monto_bs/concurrencia*especial) as monto FROM vista_liquidacion_planillas WHERE memo = ".$_GET['memo']." AND sector=".$_GET['sede']." ORDER BY sector, origen_liquidacion, anno_expediente, num_expediente;"; 
$tabla = mysql_query($consulta);
