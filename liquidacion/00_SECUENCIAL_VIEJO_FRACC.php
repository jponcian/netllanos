...
$consulta = "SELECT Max(Liquidacion.Secuencial) AS Secuencial FROM Liquidacion WHERE (((Liquidacion.Secuencial)<>999999999) AND ((Liquidacion.Serie)=41) AND ((Left([Numero_Liquidacion],4))=Year(Date())));";
$tabla = odbc_exec ($_SESSION[$_SESSION['SEDE_USUARIO']], $consulta);
$registro = odbc_fetch_object($tabla);
if (($registro->Secuencial)>0)
{$SECUENCIAL_VIEJO_FRACCIONAMIENTO=$registro->Secuencial+1;}
else 
{$SECUENCIAL_VIEJO_FRACCIONAMIENTO=1;}
// FIN
