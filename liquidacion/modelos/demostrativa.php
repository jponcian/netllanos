...
$consulta = "SELECT * FROM vista_liquidacion_planillas WHERE secuencial<>999999 AND serie<>38 and anno_expediente=".$_SESSION['ANNO_PRO']." AND num_expediente=".$_SESSION['NUM_PRO']." AND sector=".$_SESSION['SEDE']." AND origen_liquidacion=".$_SESSION['ORIGEN'];
$tabla = mysql_query($consulta); //AND id_resolucion=0 
...
{ 
	...
	if ($registro->serie==41)
		...
		{
		if ($registro->monto_ut>0 and 1==2)
	...
	if ($_SESSION['ORIGEN'] == 4)
		...
		if ($_SESSION['SEDE']==1)
			{
			$consulta1 = "SELECT tipo FROM expedientes_fiscalizacion WHERE tipo>=2000 and tipo<=2406  and anno=".$_SESSION['ANNO_PRO']." AND numero=".$_SESSION['NUM_PRO']." AND sector=1";
