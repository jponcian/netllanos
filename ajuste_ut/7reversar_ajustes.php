...
<head>
	<title>Reversar Planillas</title>
	<?php

	if ($_POST['CMDAPROBAR'] == 'Devolver') {
		$consulta = "SELECT id_expediente, num_expediente, anno_expediente, sector FROM expedientes_ajustes_ut WHERE (status>=1 and status<=6) GROUP BY rif;";
		...
		while ($registro_datos = mysql_fetch_object($tabla_datos)) {
			if ($_POST['CHECK' . ($registro_datos->id_expediente)] == 'true') {
				...
				$consulta = "UPDATE expedientes_ajustes_ut SET fecha_reverso=date(now()), status=0, usuario_reverso=" . $_SESSION['CEDULA_USUARIO'] . ", usuario=" . $_SESSION['CEDULA_USUARIO'] . " WHERE id_expediente=" . $registro_datos->id_expediente . ";";
				$tabla = mysql_query($consulta);
				// -------------- ACTUALIZACION DE LAS PLANILLAS
				$consulta = "UPDATE liquidacion SET status = 1, fecha_reverso = date(now()), usuario_reverso = " . $_SESSION['CEDULA_USUARIO'] . "  WHERE id_resolucion<=0 AND secuencial=999999 AND (origen_liquidacion=7 or origen_liquidacion=16) AND anno_expediente=" . $registro_datos->anno_expediente . " AND num_expediente=" . $registro_datos->num_expediente . " AND sector=" . $registro_datos->sector . ";";
				$tabla = mysql_query($consulta);
				// MENSAJE
	...
	<form name="form1" method="post">
		<div align="center">
						...
						<div align="center"><span class="Estilo1">
								<select name="OSEDE" size="1" onChange="this.form.submit()">
									...
									if ($_SESSION['ADMINISTRADOR'] > 0) {
										...
										$consulta_x = 'SELECT sector as id_sector, nombre FROM vista_expedientes_ajustes WHERE status=6 AND sector=' . $_SESSION['SEDE_USUARIO'] . ' GROUP BY sector;';
										$tabla_x = mysql_query($consulta_x);
										while ($registro_x = mysql_fetch_array($tabla_x)) {
											echo '<option ';
											if ($_POST['OSEDE'] == $registro_x['id_sector']) {
				...
				<tbody>
					...
					<tr>
						...
						<th width="7%" height=27>
							<div align="center" class="Estilo8"><strong>Anno</strong>:
								</td>
							</div>
						<th width="7%" height=27>
							<div align="center" class="Estilo8"><strong>Exp</strong>:
								</td>
							</div>
						<th width="7%" height=27>
							<div align="center" class="Estilo8"><strong>Rif</strong>:
								</td>
							</div>
						<th width="57%">
			...
			<p>
				...
				if ($MOSTRAR_BOTON == 'SI') {	?>
			<p>
			<div align="center"><label><span>Devolver a =>
