...
if ($_POST['CMDGUARDAR'] == "Guardar") {
	if ($_POST['ORIF'] <> "" and $_POST['OSUPERVISOR'] > 1 and $_POST['OFISCAL'] > 1 and $_POST['OSEDE'] > 0) {
		// CONSULTA DEL EXPEDIENTE SIGUIENTE
		$consulta_x = 'SELECT Max(numero)+1 AS Maximo FROM expedientes_cobro WHERE sector=0' . $_POST['OSEDE'] . ' AND anno=' . date('Y') . ';';
		...
		$consulta_existe = "SELECT Rif, Numero, Anno, Status FROM expedientes_cobro WHERE Rif='" . strtoupper($_POST['ORIF']) . "' and Anno=" . date("Y") . " and Status=0";
		...
		if ($cantidad < 1) {
			...
			$consulta = "INSERT INTO expedientes_cobro (Numero, Anno, Rif, Fecha_Registro, Coordinador, Funcionario, Usuario, Sector, Status) VALUES (" . $Maximo . ", " . date("Y") . ", '" . strtoupper($_POST['ORIF']) . "', date(now()), " . $_POST['OSUPERVISOR'] . ", " . $_POST['OFISCAL'] . ", " . $_SESSION['CEDULA_USUARIO'] . ", " . $_POST['OSEDE'] . ",0);";
			$tabla = mysql_query($consulta);
			// FIN
			echo "<script type=\"text/javascript\">alert('Expediente Creado bajo el Número => " . $Maximo . "');</script>";
		} else {
			$valor = mysql_fetch_object($tabla_existe);
			echo "<script type=\"text/javascript\">alert('Existe un Expediente abierto para el contribuyente con el Número => " . $valor->Numero . " Año => " . $valor->Anno . "');</script>";
		}
	} else {
		echo "<script type=\"text/javascript\">alert('¡¡¡Existen Campos Vacíos!!!');</script>";
	}
}
?>
<html>
	...
	<form name="form1" method="post" action="">
		<div align="center">
			<table width="50%" border="1" align="center">
				<tr>
					<td align="center" bgcolor="#FF0000" colspan="9"><span class="Estilo7"><u>Datos del Expediente - Area de Cobro </u></span></td>
				</tr>
			...
			<table width="50%" border="1" align="center">
				...
				<tr>
					...
					<td colspan="3"><label><span class="Estilo15"><?php echo $direccion;	?>
							</span></label></td>
				</tr>
			</table>
