...
<head>
	...
	if ($_POST['CMDGENERAR'] == 'Generar') {
		...
		// FIN	
		// PARA GUARDAR
		$consulta_x = "SELECT * FROM cob_actaintimacion WHERE  sector=" . $sector . " AND origen=" . $_SESSION['ORIGEN'] . " AND numero=" . $num . " AND anno=" . $anno . " AND Fecha_Resolucion>'" . $fecha_actual . "';";
		...
		if ($numero_filas > 0) {
			...
			$consulta_xx = "UPDATE cob_actaintimacion SET Planillas = '" . trim($liquidaciones) . "' WHERE sector=" . $sector . " AND origen=" . $_SESSION['ORIGEN'] . " AND numero=" . $num . " AND anno=" . $anno . ";";
			$tabla_xx = mysql_query($consulta_xx);
		} else {
			// 1ra parte
			// NUMERO DE LA RESOLUCION CONSULTA DEL MAXIMO
			$consulta_xx = "SELECT Max([NroResolucion])+1 AS Maximo FROM cob_actaintimacion WHERE AnnoResolucion=" . date('Y') . " AND sector=" . $sector . " AND origen=" . $_SESSION['ORIGEN'] . ";";
			$tabla_xx = mysql_query($consulta_xx);
			...
			$consulta_xx = "INSERT INTO cob_actaintimacion ( rif, origen, numero, anno, NroResolucion, AnnoResolucion, Fecha_Resolucion, Planillas, Usuario, sector) SELECT '" . $rif . "', " . $_SESSION['ORIGEN'] . "," . $num . ", " . $anno . ", " . $Maximo . ", " . date('Y') . ",  date(now()), '" . trim($liquidaciones) . "', '" . $_SESSION['CEDULA_USUARIO'] . "', " . $sector . ";";
			$tabla_xx = mysql_query($consulta_xx);
		}
	}
	?>
	<title>Generar Acta de Intimaci&oacute;n</title>
	<style type="text/css">
		<!--
		.Estilomenun {
...
<body style="background: transparent !important;">
	...
	<form name="form1" method="post" action="">
		...
		<table width="65%" border=1 align=center>
			...
			<?php
			$consulta = "SELECT * FROM cob_actaintimacion WHERE sector=" . $sector . " AND origen=" . $_SESSION['ORIGEN'] . " AND numero=" . $num . " AND anno=" . $anno . " ORDER BY numero;";
			$tabla = mysql_query($consulta);
			...
			?>
					<tr>
						...
						<td>
							<div align="center" class="Estilo15"><a href="Formatos/acta_de_intimacion.php?anno=<?php echo $registro->anno; ?>&num=<?php echo $registro->numero; ?>&sector=<?php echo $sector; ?>" target=_blank>VER ACTA</a></div>
						</td>
						<td colspan="1">
							<div align="center"></div>
						</td>
