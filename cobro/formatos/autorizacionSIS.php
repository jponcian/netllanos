...
class PDF extends FPDF
{
	function Footer()
	{
		...
		//Número de página
		$this->Cell(0, 0, utf8_decode(sistema()), 0, 0, 'R');
	}
}
...
list($resolucion, $fecha_res, $num_res, $anno_res) = funcion_resolucion($_SESSION['SEDE'], 13, $_SESSION['ANNO_PRO'], $_SESSION['NUM_PRO']);
// FIN

////////// INFORMACION DEL EXPEDIENTE
$consulta_datos = "SELECT * FROM vista_exp_cobro_siscontri WHERE anno=0" . $_SESSION['ANNO_PRO'] . " AND numero=0" . $_SESSION['NUM_PRO'] . " AND sector =0" . $_SESSION['SEDE'] . ";";
$tabla_datos = mysql_query($consulta_datos);
$registro_datos = mysql_fetch_object($tabla_datos);
// ---------------------

////////// SIGLAS DE LA RESOLUCION
$SIGLAS = $registro_datos->Siglas_resol_Cobro;
// ---------------------
...
$pdf->Ln(8);
$pdf->Cell(0, 5, utf8_decode('N°    ' . $registro_datos->Siglas_resol_Cobro . 'EC/' . $_SESSION['ANNO_PRO'] . '/' . $_SESSION['NUM_PRO']));
//$pdf->Ln(10);
...
$fecha_res = $registro_datos->fecha_registro;
// -----------
...
$pdf->Text($t, 24, utf8_decode($Ciudad . ', ' . strftime('%d', strtotime(date('m/d/Y', $FECHA))) . ' de ' . $mes[(strftime('%m', strtotime(date('m/d/Y', $FECHA))) - 1)] . ' del ' . strftime('%Y', strtotime(date('m/d/Y', $FECHA)))));
$pdf->Ln(13);
//----------------------- FIN

$pdf->SetFont('Times', 'B', 14 - $tamaño);

$pdf->Cell(0, 5, utf8_decode('AUTORIZACION'), 0, 0, 'C');
$pdf->Ln(10);
...
$pdf->Cell(0, 5, utf8_decode('Funcionario(s) Actuante(s)'));
$pdf->Ln(7);
$pdf->SetFont('Times', '', 8);
$pdf->Cell(30, 5, utf8_decode('Firma:'));
$pdf->Cell(0, 5, '__________________________________');
$pdf->Ln(6);
$pdf->Cell(30, 5, utf8_decode('Nombre y Apellido:'));
$pdf->Cell(95, 5, '__________________________________');
$pdf->Ln(6);
$pdf->Cell(30, 5, utf8_decode('C.I. N°:'));
$pdf->Cell(0, 5, '__________________________________');
$pdf->Ln(6);
$pdf->Cell(30, 5, utf8_decode('Fecha:'));
$pdf->Cell(0, 5, '__________________________________');
