...
$consulta_datos = "SELECT * FROM vista_exp_cobro WHERE anno=0".$_SESSION['ANNO_PRO']." AND numero=0".$_SESSION['NUM_PRO']." AND sector =0".$_SESSION['SEDE'].";";
...
$pdf->SetFont('Times','B',13-$tamaño);
	...
	$pdf->SetFont('Times','B',11);
	$pdf->SetX(60);
	$pdf->MultiCell(0,5,strtoupper(utf8_decode($registro_datos->contribuyente)));
	$pdf->Ln(3); 
	...
	$pdf->SetFont('Times','B',11);
	$pdf->SetX(60);
	$pdf->MultiCell(0,5,strtoupper(trim(utf8_decode($registro_datos->direccion))));
	...
	if ($_SESSION['ANNO_PRO'] >= 2020)
		{
$txt='En '.$registro_datos->nombre.', Estado '.$estado.', sede '.$conector1.' '.$sede.' '.$conector2.' Gerencia Regional de Tributos Internos '.utf8_decode(buscar_region()).', del Servicio Nacional Integrado de Administración Aduanera y Tributaria (SENIAT); de conformidad con lo establecido en los artículos 89, 131 numeral 2, 137 numeral 2 y 182 del Código Orgánico Tributario publicado en Gaceta Oficial N° 6.507 Extraordinario del 29 de enero de 2020, en lo sucesivo COT, la División de Recaudación adscrita a '.$adscripcion.' '.$conector2.' Gerencia Regional de Tributos Internos de la '.utf8_decode(buscar_region()).', del Servicio Nacional Integrado de Administración Aduanera y Tributaria (SENIAT), en uso de las facultades legales conferidas en el artículo 97 de la Resolución Nº 32 sobre la Organización, Atribuciones y Funciones del SENIAT, publicada en Gaceta Oficial Nº 4.881 Extraordinario, de fecha 29/03/1995 y el artículo 4 numeral 9 de la Ley del SENIAT, publicada en Gaceta Oficial Nº 37.329 de fecha 08/11/2001, el articulo 2 de la resolución SNAT/2002/Nº 913, publicada en Gaceta Oficial 37.398, de fecha 06/03/2002; se procede a emitir la presente Resolución de Imposición de Sanción, por el (la) Funcionario(a) actuante '.$registro_datos->nombrefuncionario.' titular de la cédula de Identidad N° V-'.formato_cedula($registro_datos->funcionario).' debidamente facultado(a) según Autorizacion N° '.$registro_datos->Siglas_resol_Cobro.$anno_res.'/'.($num_res).' de fecha '.voltea_fecha($fecha_res).'.';
		}
	else
		{
	$txt='En '.$registro_datos->nombre.', Estado '.$estado.', sede '.$conector1.' '.$sede.' '.$conector2.' Gerencia Regional de Tributos Internos '.utf8_decode(buscar_region()).', del Servicio Nacional Integrado de Administración Aduanera y Tributaria (SENIAT); de conformidad con lo establecido en los artículos 89, 131 numeral 2, 137 numeral 2 y 182 del Código Orgánico Tributario publicado en Gaceta Oficial N° 6.507 Extraordinario del 18 de noviembre de 2014, en lo sucesivo COT, la División de Recaudación adscrita a '.$adscripcion.' '.$conector2.' Gerencia Regional de Tributos Internos de la '.utf8_decode(buscar_region()).', del Servicio Nacional Integrado de Administración Aduanera y Tributaria (SENIAT), en uso de las facultades legales conferidas en el artículo 97  de la Resolución Nº 32 sobre la Organización, Atribuciones y Funciones del SENIAT, publicada en Gaceta Oficial Nº 4.881 Extraordinario, de fecha 29/03/1995 y el artículo 4 numeral 9 de la Ley del SENIAT, publicada en Gaceta Oficial Nº 37.329 de fecha 08/11/2001, el articulo 2 de la resolución SNAT/2002/Nº 913, publicada en Gaceta Oficial 37.398, de fecha 06/03/2002; se procede a emitir la presente Resolución de Imposición de Sanción en virtud de los hechos que se exponen a continuación:';
		}
...
$tabla_xxx = mysql_query($consulta_xxx); //echo $consulta_xxx;
...
$tabla_s = mysql_query($consulta); //echo $consulta;
//////// FIN
...
while ($registro_s = mysql_fetch_object($tabla_s))
{
	switch ($registro_s->aplicacion) 
		...
		if ($registro_s->reiteracion>1) 
			...
			{$Incremento='.';}
		//$Monto_L = 'xxxx'.num_a_letras(($registro_s->monto_bs/$registro_s->concurrencia)*$registro_s->especial);
		/////////////////////////////////
	...
	if ($registro_s->id_sancion>=20000)
		{
		$txt='Que'.utf8_decode($registro_s->sancion).', en contravención a lo establecido en el (los) artículo(s) '.trim($registro_s->art_ley_rgto).' de la (del) '.trim($registro_s->ley).' '.trim($registro_s->art_regla).', correspondiente al (a los) ejercicio(s) o (los) periodo(s) comprendido(s) entre '.voltea_fecha($registro_s->periodoinicio).' y '.voltea_fecha($registro_s->periodofinal).'; en consecuencia, esta Administración Tributaria procede a aplicar la sanción prevista en el artículo '.$Aparte.' del Código Orgánico Tributario vigente por concepto de multa en la cantidad de '.strtoupper(num_a_letras($registro_s->monto_ut)). ' ('.formato_moneda($registro_s->monto_ut).') veces el tipo de cambio oficial de la moneda de mayor valor publicado por el Banco Central de Venezuela para el momento de la infracción, la moneda de mayor valor actual es '.trim($registro_s->moneda).', por un monto de Bs. '.formato_moneda($registro_s->monto_bs/$registro_s->monto_ut).' publicado por el BCV, calculada conforme al artículo 92, establecido en el Código Organico Tributario Vigente.'; //en fecha '.voltea_fecha($registro_s->periodofinal).'
		}
	else
		{
