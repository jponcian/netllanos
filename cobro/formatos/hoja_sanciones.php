...
$consulta = "SELECT DISTINCTROW fecha_registro, NombreRazon, Rif as rif, direccion, fecha_aprobacion, anno, numero, ci_fiscal, fiscal, ci_supervisor, supervisor, sector, nombre FROM vista_hoja_sanciones_cobro WHERE anno=".$ANNO." AND numero=".$NUMERO." AND sector=".$SEDE.";";
...
$consulta_x = "SELECT Siglas_resol_Cobro FROM z_siglas WHERE id_sector=".$SEDE.";";
...
$pdf->SetFont('Times','',8);
$pdf->Cell(32,5,voltea_fecha($registro->fecha_registro),1,0,'C');
...
$consulta_x = "SELECT anno_prov, num_prov, sector, tributo, sancion, descripcion, cot, periodo, ut_aplicada, ut_concurrencia, ut_actual, total FROM vista_hoja_sanciones_fis WHERE num_prov=".$NUMERO." AND anno_prov=".$ANNO." AND sector=".$SEDE." AND origen_liquidacion=".$ORIGEN.";";
$tabla_x = mysql_query($consulta_x);
