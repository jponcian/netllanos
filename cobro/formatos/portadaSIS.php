...
$consulta = "SELECT * FROM vista_exp_cobro_siscontri WHERE sector=" . $SEDE . " and anno = " . $ANNO . " AND numero = " . $NUMERO . "";
...
if ($registro = mysql_fetch_object($tabla)) {
	...
	$ciudad = $sector . ", ";
	if ($sector > 1) {
		$dependencia = strtoupper($titulo) . ' ' . strtoupper($sector);
	} else {
		$dependencia = strtoupper($titulo) . ' ' . strtoupper(buscar_region());
	}
	...
	// Salto de línea
	$pdf->Cell(60);
	$pdf->Cell(40, 10, 'EXPEDIENTE DPP', 1, 0, 'C', 1);
	...
	$pdf->MultiCell(130, 10, utf8_decode($registro->contribuyente), 1, 'C');
	//$pdf->Ln($lineas);
	...
	$pdf->Cell(110, $lineas, utf8_decode($dependencia), 1, 0, 'C', 0);
	$pdf->Ln($lineas);
