...
if ($_POST['CMDGUARDAR'] == "Guardar") {
	$guardar = 0;
	if ($_POST['ORIF'] <> "" and $_POST['OSUPERVISOR'] > 1 and $_POST['OFUNCIONARIO'] > 1) {
		// GUARDADO DE LOS DATOS
		$consulta = "UPDATE expedientes_cobro SET rif='" . $_POST['ORIF'] . "', Coordinador=" . $_POST['OSUPERVISOR'] . ", Funcionario=" . $_POST['OFUNCIONARIO'] . ", Usuario=" . $_SESSION['CEDULA_USUARIO'] . " WHERE sector=" . $_POST['OSEDE'] . " and Numero=" . $_POST['ONUMERO'] . " and Anno=" . $_POST['OANNO'];
		//echo $consulta;
...
if ($_POST['ONUMERO'] > 0 and $_POST['ONUMERO_ANTERIOR'] <> $_POST['ONUMERO']) {
	$consulta_x = 'SELECT numero, anno, rif, coordinador, funcionario, fecha_registro FROM vista_exp_cobro WHERE sector=0' . $_POST['OSEDE'] . ' AND anno=' . $_POST['OANNO'] . ' AND numero=' . $_POST['ONUMERO'] . ';';
	...
	<form name="form1" method="post" action="">
		<div align="center">
			<table width="50%" border="1" align="center">
				<tr>
					<td height="37" colspan="9" align="center" bgcolor="#FF0000"><span class="Estilo7"><u>Datos del Expediente - Area de Cobro </u></span></td>
				</tr>
							...
							<select name="OSEDE" size="1" onChange="this.form.submit()">
								<option value="-1">--> Seleccione <--< /option>
										...
										if ($_SESSION['ADMINISTRADOR'] > 0) {
											$consulta_x = 'SELECT Sector, nombre FROM vista_exp_cobro GROUP BY sector;';
											$tabla_x = mysql_query($consulta_x);
											...
											$consulta_x = 'SELECT Sector, nombre FROM vista_exp_cobro WHERE sector=' . $_SESSION['SEDE_USUARIO'] . ' GROUP BY sector;';
											$tabla_x = mysql_query($consulta_x);
											while ($registro_x = mysql_fetch_array($tabla_x)) {
												echo '<option ';
												if ($_POST['OSEDE'] == $registro_x['sector']) {
						...
						<div align="center"><span class="Estilo1">
								<select name="OANNO" size="1" onChange="this.form.submit()">
									<option value="-1">--> Seleccione <--< /option>
											<?php
											if ($_POST['OSEDE'] > 0) {
												$consulta_x = 'SELECT anno FROM vista_exp_cobro WHERE sector =0' . $_POST['OSEDE'] . ' GROUP BY anno ORDER BY anno DESC;';
												$tabla_x = mysql_query($consulta_x);
												while ($registro_x = mysql_fetch_array($tabla_x)) {
													echo '<option ';
						...
						<div align="center"><span class="Estilo1">
								<select name="ONUMERO" size="1" onChange="this.form.submit()">
									<option value="-1">--> Seleccione <--< /option>
											<?php
											if ($_POST['OANNO'] > 0) {
												$consulta_x = 'SELECT numero FROM vista_exp_cobro WHERE sector=' . $_SESSION['SEDE_USUARIO'] . ' AND anno=' . $_POST['OANNO'] . ' ORDER BY numero DESC;';
												$tabla_x = mysql_query($consulta_x);
			...
			<table width="60%" border="1" align="center">
				...
				<tr>
					...
					<td colspan="5"><label><span class="Estilo15"><?php echo $registro_x->direccion;	?>
							</span></label></td>
				</tr>
			</table>
