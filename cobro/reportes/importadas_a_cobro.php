...
class PDF extends FPDF
	{
		function Header()
			...
			if ($_SESSION['ORIGEN_USUARIO']==0 or $_SESSION['ORIGEN_USUARIO']==13)	{	$texto1	= 'Recaudación';}
			...
			$this->Cell(0,5,'Relación de Expedientes Importados a Cobro bajo la transferencia N°: '.$_GET['memo'],0,1,'C');
			$this->Ln(4);
		...
		function Footer()
			...
			$consulta = "SELECT COUNT(DISTINCTROW sector, origen_liquidacion, anno_expediente, num_expediente) AS cantidad FROM vista_liquidacion_planillas WHERE fecha_importacion_a_cob<>'' AND memo2 = ".$_GET['memo']." AND sector=".$_GET['sede'].";";
			$tabla = mysql_query($consulta);
			if ($providencia = mysql_fetch_object($tabla)) { $cant_prov = $providencia->cantidad; };
	
			//OBTENEMOS LA CANTIDAD DE PLANILLAS
			$consulta = "SELECT COUNT(num_expediente) AS cantidad FROM vista_liquidacion_planillas WHERE fecha_importacion_a_cob<>'' AND memo2 = ".$_GET['memo']." AND sector=".$_GET['sede'].";";
			$tabla = mysql_query($consulta);
...
$consulta = "SELECT rif, contribuyente, liquidacion, periodoinicio, periodofinal, fecha_liquidacion, siglas, (monto_bs/concurrencia*especial) as monto FROM vista_liquidacion_planillas WHERE fecha_importacion_a_cob<>'' AND memo2 = ".$_GET['memo']." AND sector=".$_GET['sede']." ORDER BY contribuyente, liquidacion;"; 
$tabla = mysql_query($consulta);
...
while ($registro = mysql_fetch_object($tabla))
{
	...
	$y1=$pdf->GetY();
	//---------
	if ($y1 > 168) { $pdf->AddPage();	$pdf->Ln();		$y1=$pdf->GetY();}
	//----------------------------------
