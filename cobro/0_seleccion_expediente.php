				...
				<div align="center"><span class="Estilo1">
						<select name="OSEDE" size="1" onChange="this.form.submit()">
							...
							if ($_SESSION['ADMINISTRADOR'] > 0 or $_SESSION['SEDE_USUARIO'] == 1) {
								$consulta_x = 'SELECT sector as id_sector, nombre FROM vista_exp_cobro WHERE status>=' . $status . ' AND status<=' . $status2 . ' GROUP BY sector;';
							} else {
								$consulta_x = 'SELECT sector as id_sector, nombre FROM vista_exp_cobro WHERE status>=' . $status . ' AND status<=' . $status2 . ' AND sector=' . $_SESSION['SEDE_USUARIO'] . ' GROUP BY sector;';
							}
							//-------------
			...
			<td bgcolor="#FFFFFF">
				<div align="center"><span class="Estilo1">
						<select name="OANNO" size="1" onChange="this.form.submit()">
							...
							<?php
							if ($_POST['OSEDE'] > 0) {
								$consulta_x = 'SELECT anno FROM vista_exp_cobro WHERE status>=' . $status . ' AND status<=' . $status2 . ' AND sector=0' . $_POST['OSEDE'] . ' GROUP BY anno ORDER BY anno DESC;';
								$tabla_x = mysql_query($consulta_x);
								while ($registro_x = mysql_fetch_array($tabla_x)) {
									echo '<option ';
			...
			<td width="36%"><label>
					<div align="center"><span class="Estilo1">
							<select name="ONUMERO" size="1">
								...
								<?php
								if ($_POST['OANNO'] > 0) {
									$consulta_x = 'SELECT numero FROM vista_exp_cobro WHERE status>=' . $status . ' AND status<=' . $status2 . ' AND anno=' . $_POST['OANNO'] . ' AND sector=0' . $_POST['OSEDE'] . ' ORDER BY numero DESC;';
									$tabla_x = mysql_query($consulta_x);
									while ($registro_x = mysql_fetch_array($tabla_x)) {
										echo '<option ';
