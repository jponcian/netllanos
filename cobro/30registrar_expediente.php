...
if ($_POST['CMDGUARDAR'] == "Guardar") {
	if ($_POST['ORIF'] <> "" and $_POST['OCOORDINADOR'] > 1 and $_POST['OFISCAL'] > 1 and $_POST['OSEDE'] > 0 and $_POST['OTRIBUTO'] > 0 and $_POST['OTEXTO1'] <> '') {
		...
		$consulta_x = 'SELECT Max(numero)+1 AS Maximo FROM expedientes_especiales_siscontri WHERE sector=0' . $_POST['OSEDE'] . ' AND anno=' . date('Y') . ';';
		...
		$guardar = 0;
		$consulta_existe = "SELECT Rif, Numero, Anno, Status FROM expedientes_especiales_siscontri WHERE Rif='" . strtoupper($_POST['ORIF']) . "' and Anno=" . date("Y") . " and Status=1000";
		...
		if ($cantidad < 1) {
			...
			if ($guardar == 1) {
				...
				$fechaExpediente = $_POST['OFECHA_EXPEDIENTE'];
				$consulta = "INSERT INTO expedientes_especiales_siscontri (Numero, Anno, Rif, FechaRegistro, Coordinador, Funcionario, Usuario, Sector, Status, tributo, texto1) VALUES (" . $Maximo . ", " . date("Y") . ", '" . strtoupper($_POST['ORIF']) . "', '" . $fechaExpediente . "', " . $_POST['OCOORDINADOR'] . ", " . $_POST['OFISCAL'] . ", " . $_SESSION['CEDULA_USUARIO'] . ", " . $_POST['OSEDE'] . ",0 , '" . $_POST['OTRIBUTO'] . "', '" . $_POST['OTEXTO1'] . "');";
...
<head>
	<title>Crear Expediente SPE - SISCONTRI</title>
...
<body style="background: transparent !important;">
	...
	<form name="form1" method="post" action="">
		<div align="center">
			<table width="50%" border="1" align="center">
				<tr>
					<td height="33" colspan="9" align="center" bgcolor="#FF0000"><span class="Estilo7"><u>Datos del Expediente - SISCONTRI - Sujeto Pasivo Especial</u></span></td>
				</tr>
				...
				<td>
					<div align="center">
						<input type="date" name="OFECHA_EXPEDIENTE" id="OFECHA_EXPEDIENTE" value="<?php echo isset($_POST['OFECHA_EXPEDIENTE']) ? $_POST['OFECHA_EXPEDIENTE'] : date('Y-m-d'); ?>" required>
					</div>
				</td>
			</table>
	...
	<div align="center">
		...
		if ($_SESSION['VARIABLE'] == 'REGISTRADA') {
			...
			$consulta_x = "SELECT anno, numero FROM expedientes_especiales_siscontri WHERE sector=" . $_POST['OSEDE'] . " ORDER BY anno DESC , numero DESC;";
			$tabla_x = mysql_query($consulta_x);
			if ($registro_x = mysql_fetch_object($tabla_x)) {
				...
				//------------------
				echo '<form name="form2" method="post" action="formatos/portadaSIS.php?sede=0' . $_POST['OSEDE'] . '&num=' . $_SESSION['NUMERO'] . '&anno=' . $_SESSION['ANNO'] . '" target="_blank">';
