...
$this->Cell(160,$alto_cabecera,utf8_decode('COMPROBANTE DE REASIGNACIONES'),0,0,'C');
...
if ($_GET['comprobante']==21 or $_GET['comprobante']==31)
	{
	if ($_SESSION['DIVISION']>=100) {	$this->Cell(0,$alto_cabecera,strtoupper($area),0,0,'C');	}
		else {
			$this->Cell(0,$alto_cabecera,'GERENCIA REGIONAL DE TRIBUTOS INTERNOS '.strtoupper (utf8_decode(buscar_region())).' - '.strtoupper($division).' - '.strtoupper($area),0,0,'C');	
		}
	//$this->Cell(0,$alto_cabecera,'DIVISION DE BIENES NACIONALES',0,0,'C');
	}
else
	{
		//$this->Cell(0,$alto_cabecera,'DIVISION DE TRANSPORTE',0,0,'C');
	//---------- FILTROS
	if ($_SESSION['AREA']==0) 	//$_SESSION['AREAS'] == 'TODAS' and 
		{
		//////// ---- 
		$consulta_xxx = "SELECT id_area FROM vista_bienes_nacionales WHERE id_division=".$_SESSION['DIVISION']." AND borrado=0 ORDER BY area";
	...
	if ($_SESSION['AREAS'] <> 'ULTIMA')
		{
		$this->Cell(0,$alto_cabecera,'GERENCIA REGIONAL DE TRIBUTOS INTERNOS '.strtoupper(utf8_decode(buscar_region())).' - '.strtoupper(division($_SESSION['DIVISION'])).' - '.strtoupper(area($_SESSION['AREA_ACTUAL'])),0,0,'C');
		}
	else
		{
		$this->Cell(0,$alto_cabecera,'GERENCIA REGIONAL DE TRIBUTOS INTERNOS '.strtoupper(utf8_decode(buscar_region())).' - '.strtoupper(division($_SESSION['DIVISION'])).' - '.'RESUMEN',0,0,'C');
		}	
	//-----------------
