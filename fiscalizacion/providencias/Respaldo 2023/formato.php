...
while ($_SESSION['NUMERO']<=$_SESSION['FIN'])
	...
	$tabla = mysql_query ($consulta);
		...
		while ($x<3)
		{
			...
			$RESOLUCION = $SIGLAS."/".$_SESSION['ANNO']."/".$registro->Siglas2."/".$registro->Siglas1.sprintf("%004s", $_SESSION['NUMERO']);
			////////// FIN
			...
			$pdf->SetFont('Times','B',11);
			$pdf->Cell(0,5,'PROVIDENCIA ADMINISTRATIVA',0,0,'C'); $pdf->Ln(8);
			$pdf->SetFont('Times','',10);
