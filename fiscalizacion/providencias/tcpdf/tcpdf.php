...
class TCPDF {
	...
	public function getRawCharWidth($char) {
		...
		return $this->getABsSontMeasure($w);
	}
	...
	public function getFontBBox() {
		...
		if (isset($this->CurrentFont['desc']['FontBBox'])) {
			...
			$fbbox = array_map(array($this,'getABsSontMeasure'), $tmpbbox);
		} else {
			// Find max width
			if (isset($this->CurrentFont['desc']['MaxWidth'])) {
				$maxw = $this->getABsSontMeasure(intval($this->CurrentFont['desc']['MaxWidth']));
			} else {
				$maxw = 0;
				...
				$maxw = $this->getABsSontMeasure($maxw);
			}
	 ...
	 */
	public function getABsSontMeasure($s) {
	...
	public function getCharBBox($char) {
		...
		if (isset($this->CurrentFont['cw'][$c])) {
			...
			return array_map(array($this,'getABsSontMeasure'), $result);
		}
		return false;
	}
	...
	public function RadioButton($name, $w, $prop=array(), $opt=array(), $onvalue='On', $checked=false, $x='', $y='', $js=false) {
		...
		$popt['ap'] = array();
		$popt['ap']['n'] = array();
		$fx = ((($w - $this->getABsSontMeasure($tmpfont['cw'][108])) / 2) * $this->k);
	...
	public function CheckBox($name, $w, $checked=false, $prop=array(), $opt=array(), $onvalue='Yes', $x='', $y='', $js=false) {
		...
		$popt['ap'] = array();
		$popt['ap']['n'] = array();
		$fx = ((($w - $this->getABsSontMeasure($tmpfont['cw'][110])) / 2) * $this->k);
		$fy = (($w - ((($tmpfont['desc']['Ascent'] - $tmpfont['desc']['Descent']) * $this->FontSizePt / 1000) / $this->k)) * $this->k);
	...
	protected function getHtmlDomArray($html) {
		...
		$html = strip_tags($html, '<marker/><a><b><blockquote><body style="background: transparent !important;"><br><br/><dd><del><div><dl><dt><em><font><form><h1><h2><h3><h4><h5><h6><hr><hr/><i><img><input><label><li><ol><option><p><pre><s><select><small><span><strike><strong><sub><sup><table><tablehead><tcpdf><td><textarea><th><thead><tr><tt><u><ul>');
