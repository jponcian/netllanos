...
function Footer()
	{    
		...
		if($programa == 'Verificacion' and  $nombre == 'Calabozo' or $nombre == 'San Juan de los Morros'  or $nombre == 'San Fernando de Apure' or $nombre == 'Altagracia de Orituco' or $nombre == 'Valle de la Pascua'){
			  ...
			  elseif($programa == 'Investigacion' and  $nombre == 'Calabozo'){$Iniciales="ECAA/jac";}
               else
			  { 
		         $Iniciales="TDG/jac";
		      }
...
while ($_SESSION['NUMERO']<=$_SESSION['FIN'])
	...
	$tabla = mysql_query ($consulta);
		...
		$pdf->Cell(91,6,utf8_decode('Firmada por el Supervisor de Fiscalización'),1,0,'C');
		...
		while ($x<3)
		{
			...
			if($programa == 'Verificacion'){
				$nota=utf8_decode("Verificacion");
				
			}else{
				$nota=utf8_decode("Fiscalización y Determinación");
			}
	...
	//alex 06.2023		aplica sede	
	  ...
	  //fin alex*/
			...
			if ($_SESSION['SEDE']==1)
			{
			// $txt =$txt0 . $txt1 . utf8_decode($registro->Nombres1).' '.utf8_decode($registro->Apellidos1) . $txt2 . formato_cedula($registro->ci_fiscal1) . $txt3 . utf8_decode($registro->Nombres).' '.utf8_decode($registro->Apellidos) . $txt4 .formato_cedula($registro->ci_supervisor) . $txt5 . $txt9 . $txt6 . $txt7 . $texto_obj . $txt_after_ejercicio . $registro->texto2 . $txt_after_periodos . $txt_final;
			$txt =$txt0 . $txt1 . utf8_decode($registro->Nombres1).' '.utf8_decode($registro->Apellidos1) . $txt2 . formato_cedula($registro->ci_fiscal1) . $txt3 . utf8_decode($registro->Nombres).' '.utf8_decode($registro->Apellidos) . $txt4 .formato_cedula($registro->ci_supervisor) . $txt5 . $txt6 . $txt7 . $texto_obj . $txt_after_ejercicio . $registro->texto2 . $txt_after_periodos . $txt_final;
			
			} else {
			$txt =$txt8 . $txt0 . $txt1 . utf8_decode($registro->Nombres1).' '.utf8_decode($registro->Apellidos1) . $txt2 . formato_cedula($registro->ci_fiscal1) . $txt3 . utf8_decode($registro->Nombres).' '.utf8_decode($registro->Apellidos) . $txt4 .formato_cedula($registro->ci_supervisor) . $txt5 .  $txt9 .$txt6 . $txt7 . $texto_obj . $txt_after_ejercicio . $registro->texto2 . $txt_after_periodos . $txt_final;
			}
			//echo $txt_after_ejercicio; 
