...
class PDF extends FPDF
	{
		function Header()
		{
			...
			$this->SetXY(90,21);
			$this->Cell(0,5,mb_convert_case($area.' de Fiscalización', MB_CASE_UPPER, "ISO-8859-1"),0,1,'C');
			...
			$this->Cell(10,5,'Prov',1,0,'C',true);
		...
		$consulta = "SELECT  COUNT(DISTINCTROW numero) AS cantidad FROM vista_transferidas_fis_liquidacion WHERE fecha_transferencia_a_liq BETWEEN '".$_SESSION['INICIO']."' AND '".$_SESSION['FIN']."' AND sector=".$_SESSION['SEDE_USUARIO'].";";
		$tabla = mysql_query($consulta);
		if ($providencia = mysql_fetch_object($tabla)) { $cant_prov = $providencia->cantidad; };

		//OBTENEMOS LA CANTIDAD DE PLANILLAS
		$consulta = "SELECT  COUNT(numero) AS cantidad FROM vista_transferidas_fis_liquidacion WHERE fecha_transferencia_a_liq BETWEEN '".$_SESSION['INICIO']."' AND '".$_SESSION['FIN']."' AND sector=".$_SESSION['SEDE_USUARIO'].";";
		$tabla = mysql_query($consulta);
...
$linea = 0;
$i=1;
$consulta = "SELECT anno, numero, fecha_emision, rif, contribuyente, serie, descripcion, monto, fecha_transferencia_a_liq, sector FROM vista_transferidas_fis_liquidacion WHERE fecha_transferencia_a_liq BETWEEN '".$_SESSION['INICIO']."' AND '".$_SESSION['FIN']."' AND sector=".$_SESSION['SEDE_USUARIO']." ORDER BY anno, numero ASC;";
$tabla = mysql_query($consulta);
...
while ($registro = mysql_fetch_object($tabla))
{
	...
	$y1=$pdf->GetY();
	//---------
	if ($y1 > 170) { $pdf->AddPage(); 	$y1=$pdf->GetY();}
