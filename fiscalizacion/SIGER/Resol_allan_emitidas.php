...
$titulo = "Resoluciones de Allanamiento Emitidas";
?>
	...
	if ($row > 0) { ?>
		...
		<div>
			<table width="50%">
				...
				$sql_resumen = "SELECT a_tipo_programa.descripcion, count(a_tipo_programa.id_programa) as cantidad, sum(vista_actas_siger.impuesto_omitido) as impuesto, sum(vista_actas_siger.multa_actual) as multa, sum(vista_actas_siger.interes) as intereses, sum(vista_actas_siger.ajuste_voluntario) as pagado FROM expedientes_fiscalizacion INNER JOIN a_tipo_programa ON a_tipo_programa.id_programa = expedientes_fiscalizacion.programa INNER JOIN resoluciones ON resoluciones.id_sector = expedientes_fiscalizacion.sector AND resoluciones.anno_expediente = expedientes_fiscalizacion.anno AND resoluciones.num_expediente = expedientes_fiscalizacion.numero INNER JOIN z_siglas ON z_siglas.id_sector = expedientes_fiscalizacion.sector INNER JOIN a_tipo_providencia ON a_tipo_providencia.tipo = expedientes_fiscalizacion.tipo INNER JOIN z_sectores ON z_sectores.id_sector = expedientes_fiscalizacion.sector INNER JOIN vista_actas_siger ON vista_actas_siger.sector = expedientes_fiscalizacion.sector AND vista_actas_siger.anno_prov = expedientes_fiscalizacion.anno AND vista_actas_siger.num_prov = expedientes_fiscalizacion.numero WHERE resoluciones.id_origen = 4 AND a_tipo_programa.tipo <> 'Verificaciones' AND resoluciones.fecha BETWEEN '" . voltea_fecha($inicio) . "' AND '" . voltea_fecha($fin) . "' AND vista_actas_siger.ajuste_voluntario > 0" . $sector . " GROUP BY a_tipo_programa.id_programa";
