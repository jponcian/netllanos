...
$resolucion_prov = $SIGLAS."/".$_SESSION['ANNO_PRO']."/".$SIGLAS1.sprintf("%004s", $_SESSION['NUM_PRO']);
//$resolucion_prov = $SIGLAS."/".$_SESSION['ANNO_PRO']."/".$SIGLAS2."/".$SIGLAS1.sprintf("%004s", $_SESSION['NUM_PRO']);
//$resolucion_prov = $SIGLAS."/".$_SESSION['ANNO_PRO']."/".$SIGLAS1.sprintf("%004s", $_SESSION['NUM_PRO']);
// ---------------------

// SIGLAS ACTA Constancia
$incremento= "/____";
$acta_contancia = $SIGLAS."/".$_SESSION['ANNO_PRO']."/".$SIGLAS1.sprintf("%004s", $_SESSION['NUM_PRO']). "/___";
//FIN
...
$tabla_xcot = mysql_query($consulta_xcot);
	...
	//---------------------------------------------------------------
	$txt1 = $Sede.' Gerencia Regional de Tributos Internos '.buscar_region().' del Servicio Nacional Integrado de Administración Aduanera y tributaria (SENIAT) del Ministerio del Poder Popular de Economía, Finanzas y Comercio Exterior, ';//de conformidad con lo establecido en el Artículo 
...
if ($monto_pagado<=0)
	...
	$pdf->SetFont('Times','B',11);
	$pdf->SetX(60);
	$pdf->MultiCell(0,5,utf8_decode(trim($registro_datos->direccion)));
	
//alex 08.09.2023
//condicion
if ($registro_datos->Siglas1=='IVA/'){
	$siglasimpuesto='Impuesto al Valor Agregado';
}
elseif($registro_datos->Siglas1=='ISLR/')
{
	$siglasimpuesto='Impuesto Sobre la Renta';
}
else
{
	$siglasimpuesto=' ';
	...
	//$txt= 'De conformidad con lo establecido en los Artículos 89, 131, 133, 182 y 183 del Codigo Orgánico Tributario Vigente, en concordancia con lo dispuesto en el numeral 14 del Artículo 98 de la Resolución N° 32 de fecha 24/03/1995, sobre la organización, atribuciones y funciones del Servicio Nacional Integrado de Administración Aduanera y Tributaria SENIAT, publicada en Gaceta Oficial N° 4.881 Extraordinario de fecha 29/03/1995, Artículo 1, 2 numerales 8 y 13 de la Providencia Administrativa SNAT/2015-0009 de fecha 03-02-2015, publicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 40.598 de fecha 09-02-2015, y en el Artículo 2 de la Resolución SNAT/2002/N° 913 de fecha 06/02/2002, publicada en Gaceta Oficial N° 37.398 de fecha 06/03/2002, se procede a emitir la presente Resolución de Imposición de Sanción, por cuanto se constató para el momento de la verificación practicada el incumplimiento del (los) Deber(es) Formal(es), que se indica(n) a continuación, tal como consta en Acta de Recepción y Verificación Fiscal o Constancia de Incumplimiento levantada por el (la) Funcionario(a) actuante '.$registro_datos->Nombres1.' '.$registro_datos->Apellidos1.' titular de la cédula de Identidad N° V-'.formato_cedula($registro_datos->ci_fiscal1).' debidamente facultado(a) según Providencia Administrativa N° '.$resolucion_prov.' de fecha '.voltea_fecha($registro_datos->fecha_emision).'.';
           $txt= 'La División, de Fiscalización de la Gerencia Regional de Tributos Internos de la '.buscar_region().' del Servicio Nacional Integrado de Administración Aduanera y Tributaria (SENIAT), de conformidad con lo establecido en el Artículos 4, numerales 8, 9, 10 y 47 del Decreto N°2.177 mediante el cual el Decreto con Rango, Valor y Fuerza del la Ley del Servicio Nacional Integrado de Administración Aduanera y Tributaria - SENIAT, públicada en la Gaceta Oficial de la República Bolivariana de Venezuela N° 6.211 Extraordinario de fecha 30/12/2015; articulo 98 de la Resolución N°32 sobre la Organización, Atribuciones y Funciones del Servicio Nacional Integrado de Administración Tributaria (SENIAT) de fecha 24/03/1995, públicada en la Gaceta Oficial N° 4.881 Extraordinario de fecha 29/03/1995; los articulos 1 y 2 de la Resolución N° SNAT/2002/913 de fecha 06/02/2002; públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 37.398 de fecha 06/03/2002, y articulo 2 numerales 8 y 13 de la Providencia Administrativa N° SNAT/2015/0009 de fecha 03/02/2015, mediante el cual se reorganizan las Gerencias Regionales de Tributos Internos y las Gerencias de Aduanas Principales y se crea la División de Cobro Ejecutivo y Nedidas Cautelares de las Gerencias Regionales de Tributos Internos, públicada en Gaceta Oficial N° 40.598 de fecha 09/02/2015, en concordancia con los articulos 89, 90, 99, 131 númerales 2, 3 y 14, 133, 182 al 186 del Codigo Orgánico Tributario públicado en Gaceta Oficial de la República Bolivariana de Venezuela N° 6.507 Extraordinario de fecha 29/01/2020, mediante Providencia Administrativa Nro. '.$resolucion_prov.' de fecha '.voltea_fecha($registro_datos->fecha_emision).', notificada en fecha '.voltea_fecha($registro_datos->fecha_notificacion).', autorizó al Funcionario(a) '.$registro_datos->Nombres1.' '.$registro_datos->Apellidos1.' titular de la cédula de Identidad N° V-'.formato_cedula($registro_datos->ci_fiscal1).', bajo la Supervisión del Funcionario(a) '.$registro_datos->Nombres.' '.$registro_datos->Apellidos.' titular de la cédula de Identidad N° V-'.formato_cedula($registro_datos->ci_supervisor).', adscrito a  '.$Sede.' citada Gerencia Regional, para realizar la verificación del cumplimiento de los deberes formales en materia de '.$siglasimpuesto.', referida al Regimen de '.$registro_datos->descripcion.' para los ejercicios fiscales del '.$registro_datos->texto1.' ambos inclusive, al sujeto pasivo antes identificado.';

		   } elseif ($_SESSION['SEDE']==2 or ($_SESSION['SEDE']==3) or ($_SESSION['SEDE']==5)){
		
	//$txt= 'sjm De conformidad con lo establecido en los Artículos 89, 131, 133, 182 y 183 del Codigo Orgánico Tributario Vigente, en concordancia con lo dispuesto en el numeral 14 del Artículo 98 de la Resolución N° 32 de fecha 24/03/1995, sobre la organización, atribuciones y funciones del Servicio Nacional Integrado de Administración Aduanera y Tributaria SENIAT, publicada en Gaceta Oficial N° 4.881 Extraordinario de fecha 29/03/1995, Artículo 1, 2 numerales 8 y 13 de la Providencia Administrativa SNAT/2015-0009 de fecha 03-02-2015, publicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 40.598 de fecha 09-02-2015, y en el Artículo 2 de la Resolución SNAT/2002/N° 913 de fecha 06/02/2002, publicada en Gaceta Oficial N° 37.398 de fecha 06/03/2002, se procede a emitir la presente Resolución de Imposición de Sanción, por cuanto se constató para el momento de la verificación practicada el incumplimiento del (los) Deber(es) Formal(es), que se indica(n) a continuación, tal como consta en Acta de Recepción y Verificación Fiscal o Constancia de Incumplimiento levantada por el (la) Funcionario(a) actuante '.$registro_datos->Nombres1.' '.$registro_datos->Apellidos1.' titular de la cédula de Identidad N° V-'.formato_cedula($registro_datos->ci_fiscal1).' debidamente facultado(a) según Providencia Administrativa N° '.$resolucion_prov.' de fecha '.voltea_fecha($registro_datos->fecha_emision).'.';
	$txt= 'El Área de Fiscalización del Sector de Tributos Internos de '.$registro_datos->nombre.' adscrito a la Gerencia Regional de Tributos Internos de la '.buscar_region().' del Servicio Nacional Integrado de Administración Aduanera y Tributaria (SENIAT), de conformidad con lo establecido en el Artículos 4, numerales 8, 9, 10 y 47 del Decreto N°2.177 mediante el cual el Decreto con Rango, Valor y Fuerza del la Ley del Servicio Nacional Integrado de Administración Aduanera y Tributaria - SENIAT, públicada en la Gaceta Oficial de la República Bolivariana de Venezuela N° 6.211 Extraordinario de fecha 30/12/2015; articulo 105 y 106 de la Resolución N°32 sobre la Organización, Atribuciones y Funciones del Servicio Nacional Integrado de Administración Tributaria (SENIAT) de fecha 24/03/1995, públicada en la Gaceta Oficial N° 4.881 Extraordinario de fecha 29/03/1995; los articulos 1 y 2 de la Resolución N° SNAT/2002/913 de fecha 06/02/2002; públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 37.398 de fecha 06/03/2002, y articulo 2 numerales 8 y 13 de la Providencia Administrativa N° SNAT/2015/0009 de fecha 03/02/2015, mediante el cual se reorganizan las Gerencias Regionales de Tributos Internos y las Gerencias de Aduanas Principales y se crea la División de Cobro Ejecutivo y Nedidas Cautelares de las Gerencias Regionales de Tributos Internos, públicada en Gaceta Oficial N° 40.598 de fecha 09/02/2015, en concordancia con los articulos 89, 131 númerales 2, 3 y 14, 133, 182 al 186 del Codigo Orgánico Tributario públicado en Gaceta Oficial de la República Bolivariana de Venezuela N° 6.507 Extraordinario de fecha 29/01/2020, mediante Providencia Administrativa Nro. '.$resolucion_prov.' de fecha '.voltea_fecha($registro_datos->fecha_emision).', notificada en fecha '.voltea_fecha($registro_datos->fecha_notificacion).', autorizó al Funcionario(a) '.$registro_datos->Nombres1.' '.$registro_datos->Apellidos1.' titular de la cédula de Identidad N° V-'.formato_cedula($registro_datos->ci_fiscal1).', bajo la Supervisión del Funcionario(a) '.$registro_datos->Nombres.' '.$registro_datos->Apellidos.' titular de la cédula de Identidad N° V-'.formato_cedula($registro_datos->ci_supervisor).', adscrito al Área de Fiscalización  '.$Sede.' de la citada Gerencia Regional, para realizar la verificación del cumplimiento de los deberes formales en materia de '.$siglasimpuesto.', referida al Regimen de '.$registro_datos->descripcion.' para los ejercicios fiscales del '.$registro_datos->texto1.' ambos inclusive, al sujeto pasivo antes identificado.';

	}
	elseif ($_SESSION['SEDE']==4){
		
	//$txt= 'apure De conformidad con lo establecido en los Artículos 89, 131, 133, 182 y 183 del Codigo Orgánico Tributario Vigente, en concordancia con lo dispuesto en el numeral 14 del Artículo 98 de la Resolución N° 32 de fecha 24/03/1995, sobre la organización, atribuciones y funciones del Servicio Nacional Integrado de Administración Aduanera y Tributaria SENIAT, publicada en Gaceta Oficial N° 4.881 Extraordinario de fecha 29/03/1995, Artículo 1, 2 numerales 8 y 13 de la Providencia Administrativa SNAT/2015-0009 de fecha 03-02-2015, publicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 40.598 de fecha 09-02-2015, y en el Artículo 2 de la Resolución SNAT/2002/N° 913 de fecha 06/02/2002, publicada en Gaceta Oficial N° 37.398 de fecha 06/03/2002, se procede a emitir la presente Resolución de Imposición de Sanción, por cuanto se constató para el momento de la verificación practicada el incumplimiento del (los) Deber(es) Formal(es), que se indica(n) a continuación, tal como consta en Acta de Recepción y Verificación Fiscal o Constancia de Incumplimiento levantada por el (la) Funcionario(a) actuante '.$registro_datos->Nombres1.' '.$registro_datos->Apellidos1.' titular de la cédula de Identidad N° V-'.formato_cedula($registro_datos->ci_fiscal1).' debidamente facultado(a) según Providencia Administrativa N° '.$resolucion_prov.' de fecha '.voltea_fecha($registro_datos->fecha_emision).'.';
	$txt= 'El Área de Fiscalización de la Unidad de Tributos Internos de '.$registro_datos->nombre.' adscrito a la Gerencia Regional de Tributos Internos de la '.buscar_region().' del Servicio Nacional Integrado de Administración Aduanera y Tributaria (SENIAT), de conformidad con lo establecido en el Artículos 4, numerales 8, 9, 10 y 47 del Decreto N°2.177 mediante el cual el Decreto con Rango, Valor y Fuerza del la Ley del Servicio Nacional Integrado de Administración Aduanera y Tributaria - SENIAT, públicada en la Gaceta Oficial de la República Bolivariana de Venezuela N° 6.211 Extraordinario de fecha 30/12/2015; articulo 113 y 114 de la Resolución N°32 sobre la Organización, Atribuciones y Funciones del Servicio Nacional Integrado de Administración Tributaria (SENIAT) de fecha 24/03/1995, públicada en la Gaceta Oficial N° 4.881 Extraordinario de fecha 29/03/1995; los articulos 1 y 2 de la Resolución N° SNAT/2002/913 de fecha 06/02/2002; públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 37.398 de fecha 06/03/2002, y articulo 2 numerales 8 y 13 de la Providencia Administrativa N° SNAT/2015/0009 de fecha 03/02/2015, mediante el cual se reorganizan las Gerencias Regionales de Tributos Internos y las Gerencias de Aduanas Principales y se crea la División de Cobro Ejecutivo y Nedidas Cautelares de las Gerencias Regionales de Tributos Internos, públicada en Gaceta Oficial N° 40.598 de fecha 09/02/2015, en concordancia con los articulos 89, 131 númerales 2, 3 y 14, 133, 182 al 186 del Codigo Orgánico Tributario públicado en Gaceta Oficial de la República Bolivariana de Venezuela N° 6.507 Extraordinario de fecha 29/01/2020, mediante Providencia Administrativa Nro. '.$resolucion_prov.' de fecha '.voltea_fecha($registro_datos->fecha_emision).', notificada en fecha '.voltea_fecha($registro_datos->fecha_notificacion).', autorizó al Funcionario(a) '.$registro_datos->Nombres1.' '.$registro_datos->Apellidos1.' titular de la cédula de Identidad N° V-'.formato_cedula($registro_datos->ci_fiscal1).', bajo la Supervisión del Funcionario(a) '.$registro_datos->Nombres.' '.$registro_datos->Apellidos.' titular de la cédula de Identidad N° V-'.formato_cedula($registro_datos->ci_supervisor).', adscrito al Área de Fiscalización  '.$Sede.' de la citada Gerencia Regional, para realizar la verificación del cumplimiento de los deberes formales en materia de '.$siglasimpuesto.', referida al Regimen de '.$registro_datos->descripcion.' para los ejercicios fiscales del '.$registro_datos->texto1.' ambos inclusive, al sujeto pasivo antes identificado.';
...
if ($registro_xxx = mysql_fetch_object($tabla_xxx))
	{
	////////// IMPRESION DEL TEXTO
	$txt='En virtud de lo establecido en el Codigo Orgánico Tributario Vigente y por cuanto el contribuyente identificado supra, está calificado por la Administración tributaria como Sujeto Pasivo Especial, se procede a aumentar las sanciones pecuniarias impuestas tipificadas en el Capítulo II del citado Código en un doscientos por ciento (200%). De acuerdo al aparte único del Artículo 108 ejusdem.';
	$pdf->MultiCell(0,5, utf8_decode($txt));
	$pdf->Ln(4);
	}
...
while ($registro_s = mysql_fetch_object($tabla_s))
	...
	$Monto_L = new numerosALetras(($registro_s->monto_bs/$registro_s->concurrencia)*$registro_s->especial);
	/////////////////////////////////
	if ($registro_s->id_sancion>=20000 and ($registro_s->id_sancion<>65700) AND ($registro_s->id_sancion<>76644) and ($registro_s->id_sancion<>65701))
	
		{
		    //$txt='Que '.$registro_s->sancion.', tal y como se evidencia en el Acta Constancia N° '.$acta_contancia .' de fecha __/__/'.($registro_datos->anno).', incumpliendo lo establecido en el Árticulo 99 numeral 4 y Árticulo 155 numeral 1, literal E del '.trim($registro_s->ley).', en concordancia a lo establecido en el (los) artículo(s) '.$registro_s->art_ley_rgto.' de ***'.trim($registro_s->ley).'*** la Ley de Impuesto Sobre La Renta, Públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 6.210 Extraordinario del 30/12/2015, asi como el Árticulo '.$registro_s->art_regla.' del Reglamento de la ley de Impuesto Sobre la Renta Públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 5.662, Extraordinario de fecha 24/09/2003., concatenado con lo establecido en los Articulos 1,2 y 4 de la Providencia Administrativa N° SNAT/2010/0232 de fecha 16/04/2010, Públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 39.407 de fecha 21/04/2010, según sea aplicable en razon de su vigencia temporal; correspondiente al (a los) ejercicio(s) o (los) periodo(s) comprendido(s) entre '.voltea_fecha($registro_s->periodoinicio).' y '.voltea_fecha($registro_s->periodofinal).'; en consecuencia, esta Administración Tributaria procede a aplicar la sanción pecuniaria prevista en el artículo '.$Aparte.' del Código Orgánico Tributario vigente, incrementada segun lo establecido en la parte unico del Articulo 108 por su condicion de Sujeto Pasivo Especiales (del) '.$registro_s->ley.',  por concepto de multa. Por lo cual el Contribuyente sera Sancionado con multas de '.strtoupper(valorEnLetrasNatural(abs($registro_s->monto_ut)*$registro_s->especial)). ' ('.abs(($registro_s->monto_ut)*$registro_s->especial).') veces el tipo de cambio de la moneda de mayor valor ('.formato_moneda2($registro_s->monto_bs / $registro_s->monto_ut).'), publicada por el Banco Central De Venezuela, calculada conforme a lo establecido en la mencionada norma en virtud de no existir circunstancias atenuantes y/o agravantes que considerar en el presente caso. Asi mismo esta Admninistración Tributaria procede a la clausura de la oficina, local o establacimiento por Diez (10) dias.';
		  //$txt='Que '.$registro_s->sancion.', tal y como se evidencia en el Acta Constancia N° '.$acta_contancia .' de fecha __/__/'.($registro_datos->anno).', incumpliendo lo establecido en el Árticulo 99 numeral 4 y Árticulo 155 numeral 1, literal E del Código órganico Tributario Públicado en Gaceta Oficial Extraordinario de la República Bolivariana de Venezuela 6507 de fecha 29/01/2020, en concordancia a lo establecido en el (los) artículo(s) '.$registro_s->art_ley_rgto.' de la Ley de Impuesto Sobre La Renta, Públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 6.210 Extraordinario del 30/12/2015, asi como el Árticulo '.$registro_s->art_regla.' del Reglamento de la ley de Impuesto Sobre la Renta Públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 5.662, Extraordinario de fecha 24/09/2003., concatenado con lo establecido en los Articulos 1,2 y 4 de la Providencia Administrativa N° SNAT/2010/0232 de fecha 16/04/2010, Públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 39.407 de fecha 21/04/2010, según sea aplicable en razon de su vigencia temporal; correspondiente al (a los) ejercicio(s) o (los) periodo(s) comprendido(s) entre '.voltea_fecha($registro_s->periodoinicio).' y '.voltea_fecha($registro_s->periodofinal).'; en consecuencia, esta Administración Tributaria procede a aplicar la sanción pecuniaria prevista en el artículo '.$Aparte.' del Código Orgánico Tributario vigente, incrementada segun lo establecido en la parte unico del Articulo 108 por su condicion de Sujeto Pasivo Especiales (del) '.$registro_s->ley.',  por concepto de multa. Por lo cual el Contribuyente sera Sancionado con multas de '.strtoupper(valorEnLetrasNatural(abs($registro_s->monto_ut)*$registro_s->especial)). ' ('.abs(($registro_s->monto_ut)*$registro_s->especial).') veces el tipo de cambio de la moneda de mayor valor ('.formato_moneda2($registro_s->monto_bs / $registro_s->monto_ut).'), publicada por el Banco Central De Venezuela, calculada conforme a lo establecido en la mencionada norma en virtud de no existir circunstancias atenuantes y/o agravantes que considerar en el presente caso. Asi mismo esta Admninistración Tributaria procede a la clausura de la oficina, local o establacimiento por Diez (10) dias.';
		  $txt='Que '.$registro_s->sancion.', en contravención a lo establecido en el (los) artículo(s) '.trim($registro_s->art_ley_rgto).' de la (del) '.trim($registro_s->ley).' '.trim($registro_s->art_regla).', correspondiente al (a los) ejercicio(s) o (los) periodo(s) comprendido(s) entre '.voltea_fecha($registro_s->periodoinicio).' y '.voltea_fecha($registro_s->periodofinal).'; en consecuencia, esta Administración Tributaria procede a aplicar la sanción prevista en el artículo '.$Aparte.' del Código Orgánico Tributario vigente por concepto de multa en la cantidad de '.strtoupper(valorEnLetrasNatural($registro_s->monto_ut)). '('.abs($registro_s->monto_ut).') veces el tipo de cambio oficial de la moneda de mayor valor ('.formato_moneda2($registro_s->monto_bs / $registro_s->monto_ut).') pblicado por el Banco Central de Venezuela, calculada conforme a lo establecido en la mencionada norma en virtud de no existir circunstancias atenuantes y/o agravantes que considerar en el presente caso.';
		}
		elseif ($registro_s->id_sancion==76644)
	
		{
		     
		  $txt='Que '.$registro_s->sancion.', en contravención a lo establecido en el (los) artículo(s) '.trim($registro_s->art_ley_rgto).' de la (del) '.trim($registro_s->ley).' '.trim($registro_s->art_regla).', correspondiente al (a los) ejercicio(s) o (los) periodo(s) comprendido(s) entre '.voltea_fecha($registro_s->periodoinicio).' y '.voltea_fecha($registro_s->periodofinal).'; en consecuencia, esta Administración Tributaria procede a aplicar la sanción prevista en el artículo '.$Aparte.' de la Ley de Protecion de las Pensiones de Seguridad Social Frente al Bloqueo Imperialista por concepto de multa en la cantidad de '.strtoupper(valorEnLetrasNatural($registro_s->monto_ut)). '('.abs($registro_s->monto_ut).') veces el tipo de cambio oficial de la moneda de mayor valor ('.formato_moneda2($registro_s->monto_bs / $registro_s->monto_ut).') pblicado por el Banco Central de Venezuela, calculada conforme a lo establecido en la mencionada norma en virtud de no existir circunstancias atenuantes y/o agravantes que considerar en el presente caso.';
		}
	elseif(($registro_s->id_sancion==65700) or ($registro_s->id_sancion==65701) )
	{
	$txt='Que '.$registro_s->sancion.', tal y como se evidencia en el Acta Constancia N° '.$acta_contancia .' de fecha __/__/_______, incumpliendo lo establecido en el Árticulo 99 numeral 4 y Árticulo 155 numeral 1, literal E del Código órganico Tributario Públicado en Gaceta Oficial Extraordinario de la República Bolivariana de Venezuela N° 6.507 de fecha 29/01/2020, en concordancia a lo establecido en el (los) artículo(s) '.$registro_s->art_ley_rgto.' de la Ley de Impuesto Sobre La Renta, Públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 6.210 Extraordinario del 30/12/2015, asi como el Árticulo '.$registro_s->art_regla.' del Reglamento de la ley de Impuesto Sobre la Renta Públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 5.662, Extraordinario de fecha 24/09/2003., concatenado con lo establecido en los Articulos 1,2 y 4 de la Providencia Administrativa N° SNAT/2010/0232 de fecha 16/04/2010, Públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 39.407 de fecha 21/04/2010, según sea aplicable en razon de su vigencia; correspondiente al (a los) ejercicio(s) comprendido(s) entre '.voltea_fecha($registro_s->periodoinicio).' y '.voltea_fecha($registro_s->periodofinal).'; en consecuencia, esta Administración Tributaria procede a aplicar la sanción pecuniaria prevista en el artículo '.$Aparte.' del Código Orgánico Tributario vigente, incrementada segun lo establecido en la parte unico del Articulo 108 por su condicion de Sujeto Pasivo Especiales (del) '.$registro_s->ley.', por concepto de multa. Por lo cual el Contribuyente sera Sancionado con multas de SEIS MIL VECES(6.000,00 Veces), equivalente a '.strtoupper(valorEnLetrasNatural(abs((($registro_s->monto_ut)*$registro_s->especial)/($registro_s->concurrencia))*(($registro_s->monto_bs)/($registro_s->monto_ut)))).' Bolivares ('.formato_moneda(((($registro_s->monto_ut)*$registro_s->especial)/($registro_s->concurrencia))*(($registro_s->monto_bs)/($registro_s->monto_ut))).' Bs) al tipo de cambio de la moneda de mayor valor ('.formato_moneda2($registro_s->monto_bs / $registro_s->monto_ut).'), publicada por el Banco Central De Venezuela. Asi mismo esta Admninistración Tributaria procede a la clausura de la oficina, local o establacimiento por Diez (10) dias.';
	}
		
	elseif($registro_s->id_sancion<20000 and ($registro_s->id_sancion<>15701) and ($registro_s->id_sancion<>15700))
		{
		//$txt='Que '.$registro_s->sancion.', tal y como se evidencia en el Acta Constancia N° '.$acta_contancia .' de fecha __/__/'.($registro_datos->anno).', en contravención a lo establecido en el (los) artículo(s) '.trim($registro_s->art_ley_rgto).' de la (del) '.trim($registro_s->ley).' '.trim($registro_s->art_regla).', correspondiente al (a los) ejercicio(s) o (los) periodo(s) comprendido(s) entre '.voltea_fecha($registro_s->periodoinicio).' y '.voltea_fecha($registro_s->periodofinal).'; en consecuencia, esta Administración Tributaria procede a aplicar la sanción prevista en el artículo '.$Aparte.' del Código Orgánico Tributario vigente por concepto de multa en la cantidad de '.strtoupper(valorEnLetrasNatural($registro_s->monto_ut)). '('.abs($registro_s->monto_ut).') veces el tipo de cambio oficial de la moneda de mayor valor . ('.formato_moneda2($registro_s->monto_bs / $registro_s->monto_ut).'), publicado por el Banco Central de Venezuela, calculada conforme a lo establecido en la mencionada norma en virtud de no existir circunstancias atenuantes y/o agravantes que considerar en el presente caso.';
		//$txt='Que '.$registro_s->sancion.', tal y como se evidencia en el Acta Constancia N° '.$acta_contancia .' de fecha __/__/'.($registro_datos->anno).', en contravención a lo establecido en el (los) artículo(s) '.trim($registro_s->art_ley_rgto).' de la (del) '.trim($registro_s->ley).' '.trim($registro_s->art_regla).', correspondiente al (a los) ejercicio(s) o (los) periodo(s) comprendido(s) entre '.voltea_fecha($registro_s->periodoinicio).' y '.voltea_fecha($registro_s->periodofinal).'; en consecuencia, esta Administración Tributaria procede a aplicar la sanción prevista en el artículo '.$Aparte.' del Código Orgánico Tributario vigente por concepto de multa en la cantidad de '.strtoupper(valorEnLetrasNatural($registro_s->monto_ut)). '('.abs($registro_s->monto_ut).') veces el tipo de cambio oficial de la moneda de mayor valor . ('.formato_moneda2($registro_s->monto_bs / $registro_s->monto_ut).'), publicado por el Banco Central de Venezuela, calculada conforme a lo establecido en la mencionada norma en virtud de no existir circunstancias atenuantes y/o agravantes que considerar en el presente caso.';
	   ...
	   $txt='Que '.$registro_s->sancion.', en contravención a lo establecido en el (los) artículo(s) '.$registro_s->art_ley_rgto.' de la (del) '.$registro_s->ley.' '.$registro_s->art_regla.', correspondiente al (a los) ejercicio(s) o (los) periodo(s) comprendido(s) entre '.voltea_fecha($registro_s->periodoinicio).' y '.voltea_fecha($registro_s->periodofinal).'; en consecuencia, esta Administración Tributaria procede a aplicar la sanción prevista en el artículo '.$Aparte.' del Código Orgánico Tributario vigente por concepto de multa en la cantidad de '.abs(($registro_s->monto_ut/$registro_s->concurrencia)*$registro_s->especial).' Unidades Tributarias, calculada conforme a lo establecido en la mencionada norma en virtud de no existir circunstancias atenuantes y/o agravantes que considerar en el presente caso.';
		...
		$txt='Que '.$registro_s->sancion.', tal y como se evidencia en el Acta Constancia N° '.$acta_contancia .' de fecha __/__/______, incumpliendo lo establecido en el Árticulo 99 numeral 4 y Árticulo 155 numeral 1, literal E del Código órganico Tributario Públicado en Gaceta Oficial Extraordinario de la República Bolivariana de Venezuela 6.210 de fecha 18/11/2014, en concordancia a lo establecido en el (los) artículo(s) '.$registro_s->art_ley_rgto.' de la Ley de Impuesto Sobre La Renta, Públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 6.210 Extraordinario del 30/12/2015, asi como el Árticulo '.$registro_s->art_regla.' del Reglamento de la ley de Impuesto Sobre la Renta Públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 5.662, Extraordinario de fecha 24/09/2003., concatenado con lo establecido en los Articulos 1,2 y 4 de la Providencia Administrativa N° SNAT/2010/0232 de fecha 16/04/2010, Públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 39.407 de fecha 21/04/2010, según sea aplicable en razon de su vigencia; correspondiente al (a los) ejercicio(s) o (los) periodo(s) comprendido(s) entre '.voltea_fecha($registro_s->periodoinicio).' y '.voltea_fecha($registro_s->periodofinal).'; en consecuencia, esta Administración Tributaria procede a aplicar la sanción pecuniaria prevista en el artículo '.$Aparte.' del Código Orgánico Tributario vigente, incrementada segun lo establecido en la parte unico del Articulo 108 por su condicion de Sujeto Pasivo Especiales (del) '.$registro_s->ley.', equivalente a '.abs((($registro_s->monto_ut/$registro_s->concurrencia)*$registro_s->especial)).' Unidades Tributarias, calculada conforme a lo establecido en la mencionada norma en virtud de no existir circunstancias atenuantes y/o agravantes que considerar en el presente caso. Asi mismo esta Admninistración Tributaria procede a la clausura de la oficina, local o establacimiento por Diez (10) dias.';
		}
	
	elseif(($registro_s->id_sancion==15700) or ($registro_s->id_sancion==15701) )
		{
		//$txt='Que '.$registro_s->sancion.', tal y como se evidencia en el Acta Constancia N° '.$acta_contancia .' de fecha __/__/'.($registro_datos->anno).', en contravención a lo establecido en el (los) artículo(s) '.trim($registro_s->art_ley_rgto).' de la (del) '.trim($registro_s->ley).' '.trim($registro_s->art_regla).', correspondiente al (a los) ejercicio(s) o (los) periodo(s) comprendido(s) entre '.voltea_fecha($registro_s->periodoinicio).' y '.voltea_fecha($registro_s->periodofinal).'; en consecuencia, esta Administración Tributaria procede a aplicar la sanción prevista en el artículo '.$Aparte.' del Código Orgánico Tributario vigente por concepto de multa en la cantidad de '.strtoupper(valorEnLetrasNatural($registro_s->monto_ut)). '('.abs($registro_s->monto_ut).') veces el tipo de cambio oficial de la moneda de mayor valor . ('.formato_moneda2($registro_s->monto_bs / $registro_s->monto_ut).'), publicado por el Banco Central de Venezuela, calculada conforme a lo establecido en la mencionada norma en virtud de no existir circunstancias atenuantes y/o agravantes que considerar en el presente caso.';
		$txt='Que '.$registro_s->sancion.', tal y como se evidencia en el Acta Constancia N° '.$acta_contancia .' de fecha __/__/_____, incumpliendo lo establecido en el Árticulo 99 numeral 4 y Árticulo 155 numeral 1, literal E del Código órganico Tributario Públicado en Gaceta Oficial Extraordinario de la República Bolivariana de Venezuela 6.210 de fecha 18/11/2014, en concordancia a lo establecido en el (los) artículo(s) '.$registro_s->art_ley_rgto.' de la Ley de Impuesto Sobre La Renta, Públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 6.210 Extraordinario del 30/12/2015, asi como el Árticulo '.$registro_s->art_regla.' del Reglamento de la ley de Impuesto Sobre la Renta Públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 5.662, Extraordinario de fecha 24/09/2003., concatenado con lo establecido en los Articulos 1,2 y 4 de la Providencia Administrativa N° SNAT/2010/0232 de fecha 16/04/2010, Públicada en Gaceta Oficial de la República Bolivariana de Venezuela N° 39.407 de fecha 21/04/2010, según sea aplicable en razon de su vigencia; correspondiente al (a los) ejercicio(s) o (los) periodo(s) comprendido(s) entre '.voltea_fecha($registro_s->periodoinicio).' y '.voltea_fecha($registro_s->periodofinal).'; en consecuencia, esta Administración Tributaria procede a aplicar la sanción pecuniaria prevista en el artículo '.$Aparte.' del Código Orgánico Tributario vigente, incrementa según lo establecido del Articulo 108 por su condicion de Sujeto Pasivo Especiales del Código Orgánico Tributario del 2014 publicado en N° 6.210 Extraordinario del 18/11/2014, por concepto de multa. por lo cual el Contribuyente Sancionado con multas de SEIS MIL UNIDADES TRIBUTARIAS (6.000,00 U.T.). Equivalente a '.strtoupper(valorEnLetrasNatural((($registro_s->monto_ut/$registro_s->concurrencia)*$registro_s->especial)*(($registro_s->monto_bs)/($registro_s->monto_ut)))).' Bolivares ('.abs((($registro_s->monto_ut/$registro_s->concurrencia)*$registro_s->especial)*(($registro_s->monto_bs)/($registro_s->monto_ut))).' Bs). Asi mismo esta Admninistración Tributaria procede a la clausura de la oficina, local o establacimiento por Diez (10) dias.';
...
if ($registro_xxx = mysql_fetch_object($tabla_xxx))
	...
	//$consulta = "SELECT id_liquidacion, art_cot, ley, aplicacion, id_sancion, sum(monto_ut*especial) SumaDeUTCifras, sum(monto_ut/concurrencia*especial) as SumaDeUTdividida FROM vista_sanciones_aplicadas WHERE serie<>29 AND serie<>38 AND origen_liquidacion=0".$_SESSION['ORIGEN']." AND anno_expediente=".$_SESSION['ANNO_PRO']." AND num_expediente=".$_SESSION['NUM_PRO']." AND sector=".$_SESSION['SEDE']." GROUP BY art_cot, aplicacion, id_sancion;";
	$consulta = "SELECT id_liquidacion, art_cot, ley, aplicacion, id_sancion, sum((monto_ut/concurrencia*especial)*(monto_bs/monto_ut)) SumaDeUTCifras, sum((monto_ut/concurrencia*especial)*(monto_bs/monto_ut)) as SumaDeUTdividida FROM vista_sanciones_aplicadas WHERE serie<>29 AND serie<>38 AND origen_liquidacion=0".$_SESSION['ORIGEN']." AND anno_expediente=".$_SESSION['ANNO_PRO']." AND num_expediente=".$_SESSION['NUM_PRO']." AND sector=".$_SESSION['SEDE']." GROUP BY art_cot, aplicacion, id_sancion;";
	...
	while ($registro_r = mysql_fetch_object($tabla_r))
		...
		$pdf->Ln(3);
			...
			if (substr($registro_r->art_cot,3,1)=='#')
				...
				$Aparteley = substr($registro_r->ley,0,35);
				//----------
				if (substr($registro_r->art_cot,3,1)=="#") 
					{$Aparte = $Aparte.' Numeral '.substr($registro_r->art_cot,4,2).' '. $Aparteley; //alexito 5/12/2023
			...
			$pdf->Cell(54,5,formato_moneda($txt),1,0,'C');
			$pdf->Ln(5);	
	...
	//alex nuevo
	$consulta_x = "SELECT id_liquidacion, art_cot, ley, aplicacion, id_sancion, sum((monto_ut/concurrencia*especial)*(monto_bs/monto_ut)) SumaDeUTCifras1, sum((monto_ut/concurrencia*especial)*(monto_bs/monto_ut)) as SumaDeUTdividida1 FROM vista_sanciones_aplicadas WHERE serie<>29 AND serie<>38 AND origen_liquidacion=0".$_SESSION['ORIGEN']." AND anno_expediente=".$_SESSION['ANNO_PRO']." AND num_expediente=".$_SESSION['NUM_PRO']." AND sector=".$_SESSION['SEDE'].";";
	$tabla_x = mysql_query ( $consulta_x);
   $registro_x = mysql_fetch_object($tabla_x);
	       ...
	       $pdf->Ln(3);
			...
			$txt='                                                                                   TOTAL BS';
			$pdf->Cell(108,5,utf8_decode($txt),1,0,'C','true');
			$txt1=$registro_x->SumaDeUTCifras1;
			$pdf->Cell(2,5,'',0,0,'C');
			$pdf->Cell(54,5,formato_moneda($txt1),1,0,'C','true');
			 //alex
