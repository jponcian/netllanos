...
if ($_POST['CMDAPROBAR'] == "Devolver") {
	...
	$consulta = "UPDATE expedientes_sucesiones SET fecha_reverso=date(now()), status=0, usuario_reverso=" . $_SESSION['CEDULA_USUARIO'] . ", usuario=" . $_SESSION['CEDULA_USUARIO'] . " WHERE indice=" . $_POST['OID'] . ";";
...
<head>
	<title>Regresar Expediente Sucesiones</title>
					...
					<div align="center"><span class="Estilo1">
							<select name="OSEDE" size="1" onChange="this.form.submit()">
								...
								if ($_SESSION['ADMINISTRADOR'] > 0) {
									$consulta_x = 'SELECT sector as id_sector, nombre FROM vista_re_exp_sucesiones_liq WHERE status_planilla<11 AND (status>0 and status<=6) GROUP BY sector;';
								} else {
									$consulta_x = 'SELECT sector as id_sector, nombre FROM vista_re_exp_sucesiones_liq WHERE status_planilla<11 AND (status>0 and status<=6) and sector=' . $_SESSION['SEDE_USUARIO'] . ' GROUP BY sector;';
								}
								//------------------
				...
				<td bgcolor="#FFFFFF">
					<div align="center"><span class="Estilo1">
							<select name="OANNO" size="1" onChange="this.form.submit()">
								...
								<?php
								if ($_POST['OSEDE'] > 0) {
									$consulta_x = 'SELECT anno FROM vista_re_exp_sucesiones_liq WHERE status_planilla<11 AND (status>0 and status<=6) and sector=0' . $_POST['OSEDE'] . ' GROUP BY anno ORDER BY anno DESC;';
									$tabla_x = mysql_query($consulta_x);
									while ($registro_x = mysql_fetch_array($tabla_x)) {
										echo '<option ';
				...
				<td><label>
						<div align="center"><span class="Estilo1">
								<select name="ONUMERO" size="1" onChange="this.form.submit()">
									...
									<?php
									if ($_POST['OANNO'] > 0) {
										$consulta_x = 'SELECT numero FROM vista_re_exp_sucesiones_liq WHERE status_planilla<11 AND (status>0 and status<=6) and anno=' . $_POST['OANNO'] . ' AND sector=0' . $_POST['OSEDE'] . ' GROUP BY numero ORDER BY numero DESC;';
										$tabla_x = mysql_query($consulta_x);
										while ($registro_x = mysql_fetch_array($tabla_x)) {
											echo '<option ';
		...
		?>
			<table width="60%" border="1" align="center">
				...
				<tr>
					...
					<td><label>
							<div align="center"><span class="Estilo15"><?php echo $registro->numero; ?><input type="hidden" name="OID" value="<?php echo $registro->indice; ?>"></span>
						</label>
						</div>
					</td>
					<td bgcolor="#CCCCCC">
