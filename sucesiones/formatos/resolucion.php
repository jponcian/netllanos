...
$pdf->SetFont('Times','B',13-$tamaño);
	...
	$txt = 'En '.$registro_datos->nombre.', Estado '.$estado.', sede '.$conector1.' '.$sede.' '.$conector2.' Gerencia Regional de Tributos Internos de la '.buscar_region().', del Servicio Nacional Integrado de Administración Aduanera y Tributaria (SENIAT); de conformidad con lo establecido en los articulos 89,	131	numerales 1, 2,	3,	182 y siguentes del Codigo Organico Tributario vigente para el momento de ilicito, en concordancia con los los articulos 37, 38, 39 y 40 de la Ley de Impuesto sobre Sucesiones, Donaciones y demas Ramos Conexos vigente, el articulo 4 numerales 9 de la Ley del Servicio Nacional Integrado de Administracion Aduanera y Tributaria, publicada en la Gaceta Oficial de la Republica Bolivariana de Venezuela N° 37.320 de fecha 08/11/2001, los articulos 1 y 2 numerales 8, 13 y 47 de la Providencia Administrativa N° SNAT/2015-0009 de fecha	03/02/2015, Gaceta Oficial No	40.598	de fecha 09/02/2015, el articulo 97 numeral 21 de la Resolucion No 32 de fecha 24/03/1995, sobre la Organizacion, Atribuciones y Funciones del Servicio Nacional Integrado de Administracion Aduanera y Tributaria SENIAT, publicada en Gaceta Oficial N° 4.881 Extraordinario de fecha 29/03/1995, Providencia N° SNAT/2013/50 de Fecha 29/07/2013, asi como lo establecido en las Providencias Administrativas SNAT/2005-248 	de fecha 10-03-2005, publicada en Gaceta Oficial de la Republica Bolivariana de Venezuela No 38.243 de fecha 04-08-2005 y SNAT/2005-0954 de fecha 08-11-2005, publicada en Gaceta Oficial de la Republica Bolivariana de Venezuela N° 38.324 de fecha 29-11-2005, procede a emitir la presente Resolución, en virtud de los hechos que se exponen a continuación:';

	//$txt='En '.$registro_datos->nombre.', Estado '.$estado.', sede '.$conector1.' '.$sede.' '.$conector2.' Gerencia Regional de Tributos Internos de la '.buscar_region().', del Servicio Nacional Integrado de Administración Aduanera y Tributaria (SENIAT); de conformidad con lo establecido en los artículos 89, 131 numeral 2, 137 numeral 2 y 182 del Código Orgánico Tributario publicado en Gaceta Oficial N° 6.152 Extraordinario del 18 de noviembre de 2014 , en lo sucesivo COT, la División de Recaudación adscrita a '.$adscripcion.' '.$conector2.' Gerencia Regional de Tributos Internos de la '.buscar_region().', del Servicio Nacional Integrado de Administración Aduanera y Tributaria (SENIAT), en uso de las facultades legales conferidas en el artículo 97 numeral 21 de la Resolución Nº 32 sobre la Organización, Atribuciones y Funciones del SENIAT, publicada en Gaceta Oficial Nº 4.881 Extraordinario, de fecha 29/03/1995, procede a emitir la presente Resolución, en virtud de los hechos que se exponen a continuación:';
...
while ($registro_s = mysql_fetch_object($tabla_s))
{
	...
	if (substr($registro_s->art_cot,0,3)=='100' or substr($registro_s->art_cot,0,3)=='108' or substr($registro_s->art_cot,0,3)=='116' or substr($registro_s->art_cot,0,3)=='117')
		{	$valor = 'concatenado con el Artículo 91 del Código Organico Tributario vigente para el momento del ilícito,'; 	}
	elseif (substr($registro_s->art_cot,0,3)=='110' or substr($registro_s->art_cot,0,3)=='115' or substr($registro_s->art_cot,0,3)=='119')
		{	$valor = 'concatenado con el Artículo 92 del Código Organico Tributario vigente para el momento del ilícito,'; 	}
	else
		{	$valor = ''; 	}
	...
	if ($registro_s->id_sancion>=20000)
		...
		{
		$txt='Que '.$registro_s->sancion.', en contravención a lo establecido en el (los) artículo(s) '.$registro_s->art_ley_rgto.' de la (del) '.$registro_s->ley.' '.$registro_s->art_regla.', correspondiente al (a los) ejercicio(s) o (los) periodo(s) comprendido(s) entre '.voltea_fecha($registro_s->periodoinicio).' y '.voltea_fecha($registro_s->periodofinal).'; en consecuencia, esta Administración Tributaria procede a aplicar la sanción prevista en el artículo '.$Aparte.' del Código Orgánico Tributario vigente para el momento del ilícito, '.$valor.' por concepto de multa en la cantidad de '.abs(($registro_s->monto_ut/$registro_s->concurrencia)*$registro_s->especial).' Unidades Tributarias, calculada conforme a lo establecido en la mencionada norma en virtud de no existir circunstancias atenuantes y/o agravantes que considerar en el presente caso.';
		}
	$pdf->MultiCell(0,5,$txt);
	$pdf->Ln(4); 
	// AQUI == >>>>>>
