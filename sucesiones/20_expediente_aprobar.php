...
if ($_POST['CMDAPROBAR'] == "Aprobar") {
	...
	$consulta = "UPDATE expedientes_sucesiones SET fecha_aprobacion=date(now()), status=6, usuario=" . $_SESSION['CEDULA_USUARIO'] . " WHERE indice=" . $_POST['OID'] . ";";
	...
	<form name="form1" method="post" action="#vista">
					...
					<div align="center"><span class="Estilo1">
							<select name="OSEDE" size="1" onChange="this.form.submit()">
								...
								if ($_SESSION['ADMINISTRADOR'] > 0) {
									$consulta_x = 'SELECT sector as id_sector, nombre FROM vista_re_exp_sucesiones_liq where status=0 GROUP BY sector;';
								} else {
									$consulta_x = 'SELECT sector as id_sector, nombre FROM vista_re_exp_sucesiones_liq WHERE status=0 and sector=' . $_SESSION['SEDE_USUARIO'] . ' GROUP BY sector;';
								}
								//------------------
				...
				<td bgcolor="#FFFFFF">
					<div align="center"><span class="Estilo1">
							<select name="OANNO" size="1" onChange="this.form.submit()">
								...
								<?php
								if ($_POST['OSEDE'] > 0) {
									$consulta_x = 'SELECT anno FROM vista_re_exp_sucesiones_liq WHERE status=0 and sector=0' . $_POST['OSEDE'] . ' GROUP BY anno ORDER BY anno DESC;';
									$tabla_x = mysql_query($consulta_x);
									while ($registro_x = mysql_fetch_array($tabla_x)) {
										echo '<option ';
				...
				<td><label>
						<div align="center"><span class="Estilo1">
								<select name="ONUMERO" size="1" onChange="this.form.submit()">
									...
									<?php
									if ($_POST['OANNO'] > 0) {
										$consulta_x = 'SELECT numero FROM vista_re_exp_sucesiones_liq WHERE status=0 and anno=' . $_POST['OANNO'] . ' AND sector=0' . $_POST['OSEDE'] . ' GROUP BY numero ORDER BY numero DESC;';
										$tabla_x = mysql_query($consulta_x);
										while ($registro_x = mysql_fetch_array($tabla_x)) {
											echo '<option ';
		...
		?>
			...
			<div align="center"><label>
					<input type="submit" class="btn btn-primary" name="CMDAPROBAR" value="Aprobar">
				</label>
			</div>
			</p>
	</form>
