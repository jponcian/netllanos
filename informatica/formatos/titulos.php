<?php
//------------ CUANDO ES POR INVENTARIO
if ($comprobante == 'INVENTARIO')
{
global $a, $b, $c, $d, $e, $f;

$b=25 ; //categoria
$c=21 ; //bien	
$d=120 ; //descripcion
$e=23 ; //nombre equipo
$f=0 ; //Funcionario

$this->SetFont('Arial','B',$fuente_cabecera-0.5);

//$this->cell($a,12,'Cantidad',1,0,'C');

$x=$this->GetX();
$y=$this->GetY();
$this->multicell($c,6,'Número de Bien',1,'C');
$this->SetY($y);
$this->SetX($x+$c);

$this->cell($d,12,'DESCRIPCION',1,0,'C');

$this->cell($b,12,'Categoría',1,0,'C');

$x=$this->GetX();
$y=$this->GetY();
$this->multicell($e,6,'Nombre del Equipo',1,'C');
$this->SetY($y);
$this->SetX($x+$e);

$x=$this->GetX();
$y=$this->GetY();
$this->multicell($f,12,'Funcionario Responsable',1,'C');

//----------- VIENEN
if ($this->PageNo()>1 and $_SESSION['AREAS']<>'ULTIMA' and $_SESSION['i']>0) //
	{
	$this->SetFont('Arial','B',$fuente_cabecera);
	//$this->Cell($a,4,$_SESSION['i'],1,0,'C');
	$this->Cell(21,4,$_SESSION['i'],1,0,'C');
	$this->Cell(0,4,'VIENEN',1,0,'L');
	$this->Ln(4);
	}
//---------------
}

//------------ CUANDO ES POR REASIGNACIÓN
if ($comprobante == 'REASIGNACIÓN' or $comprobante == 'MOVIMIENTO INTERNO')
{
global $a, $b, $c, $d, $e, $f, $g, $h;

$a=15 ; //cantidad 	
$b=19 ; //codigo
$c=21 ; //bien	
$d=111 ; //descripcion
$e=15 ; //codigo
$f=20 ; //concepto
$g=20 ; //valor
$h=0 ; //total

$this->SetFont('Arial','B',$fuente_cabecera-0.5);

//$this->cell($a,12,'Cantidad',1,0,'C');

$x=$this->GetX();
$y=$this->GetY();
$this->multicell($c,4,'Número de Inventario (solo para bienes)',1,'C');
$this->SetY($y);
$this->SetX($x+$c);

$this->cell($d,12,'DESCRIPCION',1,0,'C');

$this->cell($b,12,'Categoría',1,0,'C');

$this->cell($e,12,'Código',1,0,'C');

$this->cell($f,12,'Concepto',1,0,'C');

$this->cell($g,12,'Valor Unitario',1,0,'C');

$this->cell($h,12,'Valor Total',1,0,'C');

$this->Ln(12);

//----------- VIENEN
if ($this->PageNo()>1)
	{
	$this->SetFont('Arial','B',$fuente_cabecera);
	$this->Cell($a,4,$_SESSION['i'],1,0,'C');
	$this->Cell($b,4,'',1,0,'L');
	$this->Cell($c,4,'',1,0,'L');
	$this->Cell($d,4,'VIENEN',1,0,'C');
	$this->Cell($e,4,'',1,0,'L');	
	$this->Cell($f,4,'',1,0,'L');	
	$this->Cell($g,4,'',1,0,'L');	
	$this->Cell($h,4,formato_moneda($_SESSION['monto']),1,0,'R');	
	$this->Ln(4);
	}
}
?>