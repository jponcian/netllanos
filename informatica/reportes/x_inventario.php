...
if ($_SESSION['SEDES']=='TODAS')
	...
	//------------------------
	$consulta_div = "SELECT division FROM vista_bienes_informatica GROUP BY division ORDER BY id_sector, division"; 
	...
	{
	//---------- FILTRO POR DIVISION
	if ($_SESSION['DIVISIONES']=='TODAS')
		{
		$consulta_div = "SELECT division FROM vista_bienes_informatica WHERE id_sector = ".$_SESSION['SEDE'].' GROUP BY division ORDER BY id_sector, division'; 
		//echo '<br><br> Principal => '.$consulta_div;
		$tabla_div = mysql_query($consulta_div);
		while ($registro_div = mysql_fetch_object($tabla_div))
			{
