<?php
	...
	$registros = "SELECT date_format(fecha, '%Y/%m/%d') as fecha FROM resoluciones WHERE id_sector = ".$sector." AND id_origen = 4 AND anno_expediente = ".$anno." AND num_expediente = ".$numero;
    $resultregistros = $conexionsql->query($registros);
    $rowcount=mysqli_num_rows($resultregistros);
    $valor = $resultregistros->fetch_object(); 
    $fecha = voltea_fecha($valor->fecha);
