<?php
	...
	if ($accion = 'quitar')
	{
		$registros = "UPDATE expedientes_fiscalizacion SET status=2 WHERE sector=".$sector." AND anno=".$anno." AND numero=".$numero;

		if ($resultregistros = $conexionsql->query($registros))
	    {
			$mensaje = 'Providencia modificada a estatus por notificar con éxito!!!';
			$permitido = true;
	    }
