...
<form id="form1" name="form1" method="post" action="">
	<p align="center">DEMOSTRATIVA CALCULO DE INTERESES SUMARIO ADMINISTRATIVO</p>
  <table width="769" border="1" align="center">
    <tr>
      <td width="150"><div align="center">Periodo</div></td>
      <td width="94"><div align="center">Fecha de Exigibilidad </div></td>
	  ...
	  <td width="86"><div align="center">Interes Moratorio Mesual </div></td>
...
$fecha_vencimiento = $_GET['venc'];

// CALCULO DE LOS DIAS DE MORA
list($anno,$mes,$dia)=explode('/',$fecha_pago);
$FECHA_PAGO = mktime(0,0,0,$mes,$dia,$anno);

list($anno,$mes,$dia)=explode('/',$fecha_vencimiento);
...
$FECHA_EXIGIBILIDAD = $FECHA_VENCIMIENTO;
$INTERES = 0;
$Dias = 1;
...
while ($FECHA_VENCIMIENTO <= $FECHA_PAGO)
	...
	if ($MES <> date('m',$FECHA_VENCIMIENTO))
		...
		//------------------------------------------------------------------------
		echo '<tr>
		  <td><div align="center">'.$mes_letras[number_format(doubleval($MES),0,',','.')].' - '.$AÑO.'</div></td>
		  <td><div align="center">'.date('d/m/Y',$FECHA_EXIGIBILIDAD).'</div></td>
		  <td><div align="center">'.voltea_fecha( $fecha_pago ).'</div></td>
		  <td><div align="center">'.$Dias.'</div></td>
		  <td><div align="center">'.number_format(doubleval($Monto),2,',','.').'</div></td>
		  <td><div align="center">'.number_format(doubleval($tasa * 1.20),2,',','.').'</div></td>
		  <td><div align="center">'.number_format(doubleval($INTERES),2,',','.').'</div></td>
		  <td><div align="center">'.number_format(doubleval($INTERES_ACUM),2,',','.').'</div></td>
    	</tr>';
		//------------------------------------------------------------------------
		$Dias = 0;
		$MES = date('m',$FECHA_VENCIMIENTO);
