...
class PDF extends FPDF
	{
		function Header()
		{
			...
			$this->SetXY(90,21);
			$this->Cell(0,5,mb_convert_case($area.' de Sumario Administrativo', MB_CASE_UPPER, "ISO-8859-1"),0,1,'C');

			$this->Ln(5);
			
			//TITULO DEL REPORTE
			$this->SetFont('Times','B',14);
			$this->Cell(0,5,'Relación de Expedientes Exportados a Liquidación Desde: '.date("d-m-Y",strtotime(voltea_fecha($_SESSION['INICIO']))).' Hasta: '.date("d-m-Y",strtotime(voltea_fecha($_SESSION['FIN']))),0,1,'C');
			$this->Ln(2);
			...
			$this->Cell(15,5,'Prov',1,0,'C',true);
		...
		$consulta = "SELECT  COUNT(DISTINCTROW numero) AS cantidad FROM vista_sumario_transferidas_liquidacion WHERE fecha_transferencia_a_liq BETWEEN '".voltea_fecha($_SESSION['INICIO'])."' AND '".voltea_fecha($_SESSION['FIN'])."' AND sector=".$_SESSION['SEDE'].";";
		$tabla = mysql_query($consulta);
		if ($providencia = mysql_fetch_object($tabla)) { $cant_prov = $providencia->cantidad; };

		//OBTENEMOS LA CANTIDAD DE PLANILLAS
		$consulta = "SELECT  COUNT(numero) AS cantidad FROM vista_sumario_transferidas_liquidacion WHERE fecha_transferencia_a_liq BETWEEN '".voltea_fecha($_SESSION['INICIO'])."' AND '".voltea_fecha($_SESSION['FIN'])."' AND sector=".$_SESSION['SEDE'].";";
		$tabla = mysql_query($consulta);
...
$linea = 0;
$i=1;
$consulta = "SELECT anno_expediente_fisc, num_expediente_fisc, fecha_recepcion, rif, contribuyente, serie, descripcion, monto, fecha_transferencia_a_liq, sector FROM vista_sumario_transferidas_liquidacion WHERE fecha_transferencia_a_liq BETWEEN '".voltea_fecha($_SESSION['INICIO'])."' AND '".voltea_fecha($_SESSION['FIN'])."' AND sector=".$_SESSION['SEDE']." ORDER BY anno, numero, serie, monto ASC;";
$tabla = mysql_query($consulta);
...
while ($registro = mysql_fetch_object($tabla))
{
	...
	$y1=$pdf->GetY();
	//---------
	if ($y1 > 165) { $pdf->AddPage(); 	$y1=$pdf->GetY();}
	$pdf->Cell($a,5,$registro->anno_expediente_fisc.'-'.$registro->num_expediente_fisc,$linea,0,'C'); 
	$pdf->Cell($b,5,voltea_fecha($registro->fecha_recepcion),$linea,0,'C'); 
	$pdf->Cell($c,5,$registro->rif,$linea,0,'C'); 
	//--- MULTICELL
	$x=$pdf->GetX();
	$pdf->MultiCell($d,5,substr($registro->contribuyente,0,26).'...',$linea,'J');
