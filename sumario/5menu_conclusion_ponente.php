						...
						<p class="Estilo7"><span class="Estilo1">
								<select name="OSEDE" size="1" onChange="this.form.submit()">
									...
									if ($_SESSION['ADMINISTRADOR'] > 0 or $_SESSION['SEDE_USUARIO'] == 1) {
										$consulta_x = "SELECT expedientes_sumario.sector as sector, z_sectores.nombre as dependencia FROM expedientes_sumario, z_sectores WHERE z_sectores.id_sector = expedientes_sumario.sector and expedientes_sumario.status=1 GROUP BY expedientes_sumario.sector, z_sectores.nombre;";
										...
										// -------------------------------------
										$consulta_x = "SELECT expedientes_sumario.sector as sector, z_sectores.nombre as dependencia FROM expedientes_sumario, z_sectores WHERE z_sectores.id_sector = expedientes_sumario.sector AND expedientes_sumario.sector = " . $_SESSION['SEDE_USUARIO'] . " AND expedientes_sumario.status=1 GROUP BY expedientes_sumario.sector;";
										$tabla_x = mysql_query($consulta_x);
			...
			<tbody>
				...
				if ($_POST['OSEDE'] > 0) {
					...
					while ($registro = mysql_fetch_object($tabla)) {
						...
						echo '</div></td><td ><div align="center" class="Estilo5"><div align="center"><a href="5.1conclusion_ponente.php?rif=' . $registro->rif;
						echo '&num=';
