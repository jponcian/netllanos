				...
				<div align="center"><span class="Estilo1">
						<select name="OSEDE" size="1" onChange="this.form.submit()">
							...
							if ($_SESSION['SEDE_USUARIO'] == 1) {
								$consulta_x = 'SELECT sector as id_sector, nombre FROM vista_sumario_expedientes GROUP BY sector;';
							} else {
								$consulta_x = 'SELECT sector as id_sector, nombre FROM vista_sumario_expedientes WHERE sector=' . $_SESSION['SEDE_USUARIO'] . ' GROUP BY sector;';
							}
							//--------------
			...
			<td bgcolor="#FFFFFF">
				<div align="center"><span class="Estilo1">
						<select name="OANNO" size="1" onChange="this.form.submit()">
							...
							<?php
							if ($_POST['OSEDE'] > 0) {
								$consulta_x = 'SELECT anno_expediente_fisc as anno FROM vista_sumario_expedientes WHERE sector=0' . $_POST['OSEDE'] . ' GROUP BY anno_expediente_fisc ORDER BY anno_expediente_fisc DESC;';
								$tabla_x = mysql_query($consulta_x);
								while ($registro_x = mysql_fetch_array($tabla_x)) {
									echo '<option ';
			...
			<td width="36%"><label>
					<div align="center"><span class="Estilo1">
							<select name="ONUMERO" size="1">
								...
								<?php
								if ($_POST['OANNO'] > 0) {
									$consulta_x = 'SELECT num_expediente_fisc as numero FROM vista_sumario_expedientes WHERE anno_expediente_fisc=' . $_POST['OANNO'] . ' AND sector=0' . $_POST['OSEDE'] . '  ORDER BY num_expediente_fisc DESC;';
									$tabla_x = mysql_query($consulta_x);
									while ($registro_x = mysql_fetch_array($tabla_x)) {
										echo '<option ';
