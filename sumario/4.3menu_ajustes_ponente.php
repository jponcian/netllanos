...
<head>
	...
	<title>Expedientes Ponente</title>
			...
			<tbody>
				<tr>
					<td height="45" align="center" bgcolor="#FF0000">
						<p class="Estilo7"><u>Expedientes por Concluir </u></p>
					</td>
								...
								<select name="OSEDE" size="1" onChange="this.form.submit()">
									...
									if ($_SESSION['ADMINISTRADOR'] > 0 or $_SESSION['SEDE_USUARIO'] == 1) {
										if ($_SESSION['ADMINISTRADOR'] > 0) {
											$ponente = " AND cedula_ponente<>''";
										} else {
											$ponente = " AND cedula_ponente=" . $_SESSION['CEDULA_USUARIO'];
										}

										$consulta_x = "SELECT expedientes_sumario.sector as sector, z_sectores.nombre as dependencia FROM expedientes_sumario, z_sectores WHERE z_sectores.id_sector = expedientes_sumario.sector and expedientes_sumario.status=1 " . $ponente . " GROUP BY expedientes_sumario.sector, z_sectores.nombre;";
										...
										$ponente = " AND cedula_ponente=" . $_SESSION['CEDULA_USUARIO'];
										// -------------------------------------
										$consulta_x = "SELECT expedientes_sumario.sector as sector, z_sectores.nombre as dependencia FROM expedientes_sumario, z_sectores WHERE z_sectores.id_sector = expedientes_sumario.sector AND expedientes_sumario.sector = " . $_SESSION['SEDE_USUARIO'] . " AND expedientes_sumario.status=1 " . $ponente . " GROUP BY expedientes_sumario.sector;";
										$tabla_x = mysql_query($consulta_x);
		...
		<table width="55%" border=1 align=center>
			<tbody>
				...
				if ($_POST['OSEDE'] > 0) {
					...
					$consulta_x = "SELECT vista_contribuyentes_direccion.rif, vista_contribuyentes_direccion.contribuyente, expedientes_sumario.anno, expedientes_sumario.anno_expediente_fisc AS anno_exp, expedientes_sumario.num_expediente_fisc AS num_exp, expedientes_sumario.numero, a_origen_liquidacion.Descripcion AS area, z_sectores.nombre, expedientes_sumario.origen_liquidacion, expedientes_sumario.sector, CONCAT_WS(' ',z_empleados.Nombres,z_empleados.Apellidos) as ponente FROM expedientes_sumario INNER JOIN vista_contribuyentes_direccion ON vista_contribuyentes_direccion.rif = expedientes_sumario.rif INNER JOIN a_origen_liquidacion ON a_origen_liquidacion.Codigo = expedientes_sumario.origen_liquidacion INNER JOIN z_sectores ON z_sectores.id_sector = expedientes_sumario.sector INNER JOIN z_empleados ON z_empleados.cedula = expedientes_sumario.cedula_ponente WHERE expedientes_sumario.sector = " . $_POST['OSEDE'] . " AND expedientes_sumario.status = 1" . $ponente . " ORDER BY rif ASC"; //echo $consulta_x;
					...
					while ($registro = mysql_fetch_object($tabla)) {
						...
						echo '</div></td><td ><div align="center" class="Estilo5"><div align="center"><a href="4.3ajustes_ponente.php?rif=' . $registro->rif;
						echo '&num=';
