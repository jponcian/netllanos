...
<body style="background: transparent !important;">
	<p>
		...
		if ($_POST['CMDCONCLUIR'] == "Concluir") {
			...
			//-----------
			if ($_POST['OFECHA'] <> "") {
				if (strtotime(voltea_fecha($_POST['OFECHA'])) < strtotime($fecha_registro)) {
					...
					if ($status_prov == 0) {
						...
						$consulta = "UPDATE expedientes_especiales SET Status=4, fecha_conclusion = '" . voltea_fecha($_POST['OFECHA']) . "', Usuario=" . $_SESSION['CEDULA_USUARIO'] . " WHERE anno=" . $_SESSION['ANNO_PRO'] . " AND numero=" . $_SESSION['NUM_PRO'] . " AND Sector=" . $_SESSION['SEDE'] . ";";
	...
	<form name="form1" method="post" action="#vista">
		<table width="47%" border="1" align="center">
			<tr>
				<td height="35" align="center" bgcolor="#FF0000" colspan="6"><span class="Estilo7"><u>Selecci&oacute;n del Expediente a Concluir </u></span></td>
			</tr>
			<tr>
				<td bgcolor="#CCCCCC">
					...
					<div align="center"><span class="Estilo1">
							<select name="OSEDE" size="1" onChange="this.form.submit()">
								...
								if ($_SESSION['ADMINISTRADOR'] > 0) {
									$consulta_x = 'SELECT sector as id_sector, nombre FROM vista_exp_especiales WHERE Status=0 GROUP BY sector;';
								} else {
									$consulta_x = 'SELECT sector as id_sector, nombre FROM vista_exp_especiales WHERE status=0 and sector=' . $_SESSION['SEDE_USUARIO'] . ' GROUP BY sector;';
								}
								//-------
				...
				<td bgcolor="#FFFFFF">
					<div align="center"><span class="Estilo1">
							<select name="OANNO" size="1" onChange="this.form.submit()">
								...
								<?php
								if ($_POST['OSEDE'] > 0) {
									$consulta_x = 'SELECT anno FROM vista_exp_especiales WHERE status=0 AND sector=0' . $_POST['OSEDE'] . ' GROUP BY anno ORDER BY anno DESC;';
									$tabla_x = mysql_query($consulta_x);
									while ($registro_x = mysql_fetch_array($tabla_x)) {
										echo '<option ';
				...
				<td width="36%"><label>
						<div align="center"><span class="Estilo1">
								<select name="ONUMERO" size="1" onChange="this.form.submit()">
									...
									<?php
									if ($_POST['OANNO'] > 0) {
										$consulta_x = 'SELECT numero FROM vista_exp_especiales WHERE status=0 AND anno=' . $_POST['OANNO'] . ' AND sector=0' . $_POST['OSEDE'] . ' ORDER BY numero DESC;';
										$tabla_x = mysql_query($consulta_x);
										while ($registro_x = mysql_fetch_array($tabla_x)) {
											echo '<option ';
			...
			<table width="30%" border="1" align="center">
				...
				<tr>
					...
					<td align="center"><label>
							<input type="text" name="OFECHA" size="12" value="<?php echo date("d/m/Y"); ?>" readonly=""></label></td>
