...
<body style="background: transparent !important;">
	...
	if ($_POST['CMDTRANSFERIR'] == 'Transferir') {
		$consulta = "SELECT * FROM vista_exp_especiales WHERE sector=0" . $_POST['OSEDE'] . " AND anno=0" . $_POST['OANNO'] . " AND status=6 AND coordinador<>0;";
		...
		while ($registro_datos = mysql_fetch_object($tabla_datos)) {
			if ($_POST['CHECK' . $registro_datos->id_expediente] == $registro_datos->id_expediente) {
				...
				$consulta = "UPDATE expedientes_especiales SET status=7, fecha_transferencia = date(now()) WHERE id_expediente=" . $registro_datos->id_expediente . ";";
				$tabla = mysql_query($consulta);
				// --------------

				// -------------- ACTUALIZACION DE LA LIQUIDACION
				$consulta = "UPDATE liquidacion SET status = 10, fecha_transferencia_a_liq = date(now()), usuario_transferencia_a_liq=" . $_SESSION['CEDULA_USUARIO'] . ", usuario=" . $_SESSION['CEDULA_USUARIO'] . " WHERE id_resolucion<=0 AND sector=" . $registro_datos->sector . " AND anno_expediente=" . $registro_datos->anno . " AND num_expediente=" . $registro_datos->numero . " AND origen_liquidacion=" . $_SESSION['ORIGEN'] . ";";
				$tabla = mysql_query($consulta);
				// --------------
						...
						<div align="center"><span class="Estilo1">
								<select name="OSEDE" size="1" onChange="this.form.submit()">
									...
									if ($_SESSION['ADMINISTRADOR'] > 0) {
										$consulta_x = 'SELECT sector as id_sector, nombre FROM vista_exp_especiales WHERE status=6 GROUP BY sector;';
										...
										$consulta_x = 'SELECT sector as id_sector, nombre FROM vista_exp_especiales WHERE status=6 AND sector=' . $_SESSION['SEDE_USUARIO'] . ' GROUP BY sector;';
										$tabla_x = mysql_query($consulta_x);
										while ($registro_x = mysql_fetch_array($tabla_x)) {
											echo '<option ';
											if ($_POST['OSEDE'] == $registro_x['id_sector']) {
					...
					<td bgcolor="#FFFFFF">
						<div align="center"><span class="Estilo1">
								<select name="OANNO" size="1" onChange="this.form.submit()">
									...
									<?php
									if ($_POST['OSEDE'] > 0) {
										$consulta_x = 'SELECT anno FROM vista_exp_especiales WHERE status=6 AND sector=0' . $_POST['OSEDE'] . ' GROUP BY anno;';
										$tabla_x = mysql_query($consulta_x);
										while ($registro_x = mysql_fetch_array($tabla_x)) {
											echo '<option ';
				...
				<tbody>
					<tr>
						<td bgcolor="#FF0000" height="27" colspan="10" align="center">
							<p class="Estilo7"><u>Datos de la(s) Expedientes(es) por Transferir </u></p>
						</td>
					</tr>
					<tr>
					...
					$i = 0;
					//------
					$consulta = "SELECT id_expediente, anno, numero, nombrefuncionario, nombrecoordinador, FechaRegistro, contribuyente FROM vista_exp_especiales WHERE sector=0" . $_POST['OSEDE'] . " AND anno=0" . $_POST['OANNO'] . " AND status=6 AND coordinador<>0 ORDER BY id_expediente DESC;"; //echo $consulta;
					$tabla_datos = mysql_query($consulta);
					while ($registro_datos = mysql_fetch_object($tabla_datos)) {
						$i++;
						//--------
