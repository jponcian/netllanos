...
if ($_POST['CMDAPROBAR'] == "Devolver") {
	// ACTUALIZACION DEL STATUS DEL EXPEDIENTE
	$consulta = "UPDATE expedientes_especiales SET fecha_reverso=date(now()), status=0, usuario_reverso=" . $_SESSION['CEDULA_USUARIO'] . ", usuario=" . $_SESSION['CEDULA_USUARIO'] . " WHERE id_expediente=" . $_POST['OID'] . ";";
...
<head>
	<title>Regresar Expediente Especiales</title>
...
<body style="background: transparent !important;">
	...
	<form name="form1" method="post" action="#vista">
		<table width="47%" border="1" align="center">
			<tr>
				<td height="35" align="center" bgcolor="#FF0000" colspan="6"><span class="Estilo7"><u>Selecci�n del Expediente a Reversar</u></span></td>
			</tr>
			<tr>
				<td bgcolor="#CCCCCC">
					...
					<div align="center"><span class="Estilo1">
							<select name="OSEDE" size="1" onChange="this.form.submit()">
								...
								if ($_SESSION['ADMINISTRADOR'] > 0) {
									$consulta_x = 'SELECT sector as id_sector, nombre FROM vista_exp_especiales_liq WHERE status_planilla<11 AND (status>=4 and status<=6) GROUP BY sector;';
								} else {
									$consulta_x = 'SELECT sector as id_sector, nombre FROM vista_exp_especiales_liq WHERE status_planilla<11 AND (status>=4 and status<=6) and sector=' . $_SESSION['SEDE_USUARIO'] . ' GROUP BY sector;';
								}
								//------------------
				...
				<td bgcolor="#FFFFFF">
					<div align="center"><span class="Estilo1">
							<select name="OANNO" size="1" onChange="this.form.submit()">
								...
								<?php
								if ($_POST['OSEDE'] > 0) {
									$consulta_x = 'SELECT anno FROM vista_exp_especiales_liq WHERE status_planilla<11 AND (status>=4 and status<=6) and sector=0' . $_POST['OSEDE'] . ' GROUP BY anno ORDER BY anno DESC;';
									$tabla_x = mysql_query($consulta_x);
									while ($registro_x = mysql_fetch_array($tabla_x)) {
										echo '<option ';
				...
				<td><label>
						<div align="center"><span class="Estilo1">
								<select name="ONUMERO" size="1" onChange="this.form.submit()">
									...
									<?php
									if ($_POST['OANNO'] > 0) {
										$consulta_x = 'SELECT numero FROM vista_exp_especiales_liq WHERE status_planilla<11 AND (status>=4 and status<=6) and anno=' . $_POST['OANNO'] . ' AND sector=0' . $_POST['OSEDE'] . ' GROUP BY numero ORDER BY numero DESC;';
										$tabla_x = mysql_query($consulta_x);
										while ($registro_x = mysql_fetch_array($tabla_x)) {
											echo '<option ';
		...
		?>
			<table width="60%" border="1" align="center">
				...
				<tr>
					...
					<td><label>
							<div align="center"><span class="Estilo15"><?php echo $registro->numero; ?><input type="hidden" name="OID" value="<?php echo $registro->id_expediente; ?>"></span>
						</label>
						</div>
					</td>
					<td bgcolor="#CCCCCC">
			...
			<p>
			<div align="center"><label><span class="Estilo1">Devolver a =>
						<select name="OESTATUS" size="1" onChange="this.form.submit()">
							<option value="0">Funcionario</option>
						</select>
					</span>
					<input type="submit" class="boton" name="CMDAPROBAR" value="Devolver">
				</label>
			</div>
			</p>
