...
if ($_POST['ONUMERO'] > 0) {
	$consulta_x = 'SELECT numero, anno, rif, coordinador, funcionario, FechaRegistro, Especial FROM vista_exp_especiales WHERE sector=0' . $_POST['OSEDE'] . ' AND anno=' . $_POST['OANNO'] . ' AND numero=' . $_POST['ONUMERO'] . ';';
...
?>
<html>
<title>Reimprimir Portada de Expediente SPE</title>
	...
	<form name="form1" method="post" action="">
		<div align="center">
			<table border="1" align="center">
								...
								<select name="ONUMERO" size="1" onChange="this.form.submit()">
									...
									if ($_POST['OANNO'] > 0) {
										if ($_SESSION['ADMINISTRADOR'] > 0) {
											...
											$consulta_x = 'SELECT numero FROM vista_exp_especiales WHERE sector=' . $_SESSION['SEDE_USUARIO'] . ' AND anno=' . $_POST['OANNO'] . ' ORDER BY numero DESC;';
										}
										//---------------
										$tabla_x = mysql_query($consulta_x);
										while ($registro_x = mysql_fetch_array($tabla_x)) {
				...
				<?php
				if ($_POST['ONUMERO'] > 0) {
					$consulta_x = 'SELECT fecha_registro, rif FROM vista_exp_especiales WHERE sector=' . $_SESSION['SEDE_USUARIO'] . ' AND anno=' . $_POST['OANNO'] . ' AND numero=' . $_POST['ONUMERO'] . ';';
					$tabla_x = mysql_query($consulta_x);
					$registro_x = mysql_fetch_array($tabla_x);
					//----------
					$_POST['OFECHA'] = $registro_x['fecha_registro'];
					$_POST['ORIF'] = $registro_x['rif'];
				}
				?>
				<td><label>
						<div align="center">
							<input type="text" name="OFECHA" value="<?php echo $_POST['OFECHA']; ?>" readonly>
						</div>
					</label></td>
			</table>
			<?php
