...
class PDF extends FPDF
	{
		...
		$consulta = "SELECT  COUNT(DISTINCTROW numero) AS cantidad FROM vista_transferidas_esp_liquidacion WHERE id_resolucion<=0 AND fecha_transferencia_a_liq BETWEEN '".$_SESSION['INICIO']."' AND '".$_SESSION['FIN']."' AND sector=".$_SESSION['SEDE'].";";
		$tabla = mysql_query($consulta);
		if ($providencia = mysql_fetch_object($tabla)) { $cant_prov = $providencia->cantidad; };

		//OBTENEMOS LA CANTIDAD DE PLANILLAS
		$consulta = "SELECT  COUNT(numero) AS cantidad FROM vista_transferidas_esp_liquidacion WHERE id_resolucion<=0 AND fecha_transferencia_a_liq BETWEEN '".$_SESSION['INICIO']."' AND '".$_SESSION['FIN']."' AND sector=".$_SESSION['SEDE'].";";
		$tabla = mysql_query($consulta);
...
$linea = 0;
$i=1;
$consulta = "SELECT anno, numero, fecha_emision, rif, contribuyente, serie, descripcion, monto, fecha_transferencia_a_liq, sector FROM vista_transferidas_esp_liquidacion WHERE historial<=0 AND id_resolucion<=0 AND fecha_transferencia_a_liq BETWEEN '".$_SESSION['INICIO']."' AND '".$_SESSION['FIN']."' AND sector=".$_SESSION['SEDE']." ORDER BY anno, numero, serie, monto ASC;";
//echo $consulta;
$tabla = mysql_query($consulta);
