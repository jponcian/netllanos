...
class PDF extends FPDF
{
	...
	function cabeceraHorizontal($cabecera, $x, $y)
    {
		...
		*/
            $this->CellFitSpace(175,5, utf8_decode($cabecera[0]),1, 0 , 'L', true);
    ...
    function datosHorizontal($datos, $x, $y)
    {
        ...
        foreach($datos as $fila)
        {
            ...
            $this->CellFitSpace(100,5, utf8_decode($fila['BANCO']),1, 0 , 'L', $bandera );
			$this->CellFitSpace(75,5, utf8_decode($fila['BANCO1']),1, 0 , 'L', $bandera );
...
//		}
	...
	//CALCULAR EL VALOR DE LA UNIDAD TRIBUTARIA PARA EL MOMENTO DEL CIERRE DEL PERIODO (MONEDA DEL MAYOR VALOR) 2023 ALEX
	...
	$i=0;
	$sqlbancos = "SELECT a_banco.Descripcion FROM a_banco order by a_banco.caracter DESC, a_banco.id_banco";
	$tabla_bancos = mysql_query ($sqlbancos);
	    while ($registro_bancos = mysql_fetch_object($tabla_bancos))
		{
		if ($i>0)	{	$bancos = $bancos.', ';	}			
		$bancos = $bancos.$registro_bancos->Descripcion;
		$i++;
		}
	...
	//$txt7 = ("1. Deberá cumplir con sus obligaciones de declarar y pagar los tributos administrados por este Servicio, exclusivamente en las siguientes Oficinas Receptoras de Fondos Nacionales, únicos autorizados para recibir todos los pagos de sus obligaciones tributarias a traves de todas las agencias de la banca pública y privada ($bancos) de su jurisdicción.");
	...
	/*$txt12 = ("a. En Efectivo.
b. Cheque a nombre del Tesoro Nacional del Banco del Tesoro, Banco de Venezuela y Banco Bicentenario.
...
*/
$txt12 = ("a. En Efectivo.
b. Cheque a nombre del Tesoro Nacional en cualquiera de los bancos recaudadores tantos de las banca Pública como Privada.
c. Cheque de Gerencia de otros Bancos a nombre del Tesoro Nacional.
d. Pago electrónico a través de cualquiera de las instituciones Financieras.
e. Titulos valores, los cuales deberán ser transferidos previo a la fecha en que se realizará el pago, según la normativa vigente.");


	$txt17 = ("3.  Los pagos deben ser realizados de lunes a viernes en horario corrido, comprendido de 8:30 a.m. a 3:30 p.m., exceptuando los días feriados nacionales, regionales o bancarios, y para consultas y demás trámites, en horario comprendido de 8:00 a.m. a 4:00 p.m.  Adicionalmente podrá realizar los pagos en linea, a través de cualquier plataforma
	bancaria autorizada por el Servicio Nacional Integrado de Administración Aduanera y Tributaria (SENIAT).");
	...
	//$txt19 = ("5. Por ser sujeto pasivo especial, debe regirse por el calendario especial para el cumplimiento de sus obligaciones tributarias, cuya fecha de vencimiento se ha diseñado en función del último dígito de su Registro Único de Información Fiscal (RIF), establecido en la Providencia Administrativa SNAT/2018/0189  de fecha 04/12/2018, Publicado en Gaceta Oficial de la República Bolivariana de Venezuela Nº 41.456 del 14/12/2018, así como en los ejercicios  sucesivos, los  cuales  serán publicados  en  la Gaceta Oficial de la República Bolivariana de Venezuela durante los últimos días de cada año calendario, por lo  cual  deberá estar atento en los medios impresos y a nuestro Portal  Fiscal www.seniat.gob.ve.");
	
	$txt19 = ("5. Por ser sujeto pasivo especial, debe regirse por el calendario especial para el cumplimiento de sus obligaciones tributarias, cuya fecha de vencimiento se ha diseñado en función del último dígito de su Registro Único de Información Fiscal (RIF), tipificado en la Providencia Administrativa vigente que establece el calendario de Sujetos Pasivos Especiales y Agente de Retención, asi como en los ejercicios sucesivos, los  cuales  serán publicados  en  la Gaceta Oficial de la República Bolivariana de Venezuela durante los últimos días de cada año calendario, por lo  cual  deberá estar atento en los medios impresos y a nuestro Portal  Fiscal www.seniat.gob.ve.");
	...
	//$pdf->Ln(3); 
	//$pdf->SetXY($x,$y);
	//$miCabecera = array('                                                                                                 BANCA PUBLICA');
 
	//$sqlbancos = "SELECT a_banco.Descripcion FROM a_banco WHERE a_banco.caracter='PUBLICO' order by a_banco.id_banco asc limit 2 ";
	//$sqlbancos1 = "SELECT a_banco.Descripcion FROM a_banco WHERE a_banco.caracter='PUBLICO' order by a_banco.id_banco desc limit 2 ";
	//$pdf->cabeceraHorizontal($miCabecera, $x, $y);
	//$tabla_bancos = mysql_query ($sqlbancos);
	//$tabla_bancos1 = mysql_query ($sqlbancos1);
	 //   while ($registro_bancos = mysql_fetch_object($tabla_bancos) and $registro_bancos1 = mysql_fetch_object($tabla_bancos1))
	//	{
	//	$misDatos = array(
	//					array('BANCO' => $registro_bancos->Descripcion,'BANCO1' => $registro_bancos1->Descripcion,),
	...
	//$miCabecera = array('                                                                                                  BANCA PRIVADA');
	

	//$sqlbancos = "SELECT a_banco.Descripcion FROM a_banco WHERE a_banco.caracter='PRIVADO'order by a_banco.id_banco asc limit 6";
	//$sqlbancos1 = "SELECT a_banco.Descripcion FROM a_banco WHERE a_banco.caracter='PRIVADO' order by a_banco.id_banco desc limit 6";
	//$pdf->cabeceraHorizontal($miCabecera, $x, $y);
	//$tabla_bancos = mysql_query ($sqlbancos);
	//$tabla_bancos1 = mysql_query ($sqlbancos1);
	...
	$pdf->Cell(0,10,'FUNCIONARIO NOTIFICADOR',0,1,'L');
	$pdf->SetFont('Times','',8);
