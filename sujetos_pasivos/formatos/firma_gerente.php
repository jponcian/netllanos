...
//--------------------
	...
	if ($registro_x->sector==1)
	{
		...
		$pdf->SetFont('Times','',8);
		$pdf->MultiCell(0,4,$providencia.', de fecha '.date("d/m/Y", strtotime($fecha_prov)),0,'C');
		//$pdf->MultiCell(0,4,'de fecha '.date("d/m/Y", strtotime($fecha_prov)),0,'C');
		$pdf->MultiCell(0,4,$gaceta.', de fecha '.(voltea_fecha($fecha_gac)),0,'C');
	}
	else
	{
		$pdf->SetFont('Times','',9);
