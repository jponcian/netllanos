			...
			if ($_POST['OTIPO'] > 0 and $_POST['OSEDE'] > 0 and $_POST['OBANCO'] > 0 and $_POST['OPERIODO'] <> '-1' and $_POST['OQUINCENA'] <> '-1' and $_POST['ONUMERO'] > 0 and $_POST['OFECHA_PAGO'] <> "" and $_POST[OFECHA_PRE] <> "" and $_POST['OMONTO'] > 0) {
				...
				if ($registro_x = mysql_fetch_object($tabla_x)) {
					...
					if (($_GET['periodo'] <> "" or $_POST['OPERIODO'] <> "-1") and ($_POST['OQUINCENA'] <> "-1" and substr($_POST['ORIF'], 9, 1) <> "")) {
						...
						$tabla_x = mysql_query($consulta_x);
						if ($registro_x = mysql_fetch_object($tabla_x)) {
							if ($registro_x->Fecha <> '0000/00/00' and $registro_x->Fecha <> '') {
								$Fecha_Ven = $registro_x->Fecha;
							} else {
								if ($_POST['OFECHA_VEN'] <> '') {
									$Fecha_Ven = voltea_fecha($_POST['OFECHA_VEN']);
								} else {
				...
				<td width="357" bgcolor="#FFFFFF">
					<div align="center">
						...
						<select name="OBANCO" size="1">
							<option value="-1">Seleccione</option>
							<?php
							$consulta_x = "SELECT * FROM vista_ce_consulta_banco WHERE sector=0" . $_POST['OSEDE'] . " ORDER BY id_agencia;";
							$tabla_x = mysql_query($consulta_x);
							while ($registro_x = mysql_fetch_object($tabla_x)) {
								echo '<option';
				...
				<td width="134" bgcolor="#FFFFFF">
					<div align="center">
						<select name="OANNO" size="1" onChange="this.form.submit()">
							...
							$consulta_x = "SELECT RIGHT(Periodo,4) AS anno FROM CE_Calendario WHERE Tipo_Impuesto=0" . $Tipo . " GROUP BY RIGHT(Periodo,4) ORDER BY RIGHT(Periodo,4) DESC;";
							$tabla_x = mysql_query($consulta_x);
					...
					<div align="center">
						<select name="OPERIODO" size="1" onChange="this.form.submit()">
							<option value="-1">-> Seleccione <-< /option>
									...
									$consulta_x = "SELECT CE_Calendario.Periodo FROM CE_Calendario WHERE (((CE_Calendario.Tipo_Impuesto)=0" . $Tipo . ")) GROUP BY CE_Calendario.Periodo, RIGHT(Periodo,4) HAVING (((RIGHT(Periodo,4))=0" . $_POST['OANNO'] . ")) ORDER BY CE_Calendario.Periodo DESC , RIGHT(Periodo,4) DESC;";
									$tabla_x = mysql_query($consulta_x);
					...
					<div align="center">
						<select name="OQUINCENA" size="1" onChange="this.form.submit()">
							<option value="-1">-> Seleccione <-< /option>
									...
									$consulta_x = "SELECT Quincena FROM CE_Calendario WHERE Tipo_Impuesto=0" . $Tipo . " GROUP BY Quincena ORDER BY Quincena DESC;";
									$tabla_x = mysql_query($consulta_x);
									while ($registro_x = mysql_fetch_object($tabla_x)) {
										echo '<option';
					...
					<div align="center">
						<input onclick='javascript:scwShow(this,event);' type="text" name="OFECHA_VEN" size="8" readonly value="<?php
																																...
																																if (($_GET['periodo'] <> "" or $_POST['OPERIODO'] <> "-1") and ($_POST['OQUINCENA'] <> "-1" and substr($_POST['ORIF'], 9, 1) <> "")) {
																																	...
																																	if ($registro_x = mysql_fetch_object($tabla_x)) {
																																		if ($registro_x->Fecha <> '0000/00/00' and $registro_x->Fecha <> '' and $registro_x->Fecha <> 'null') {
																																			$Fecha_Ven = voltea_fecha($registro_x->Fecha);
																																		} else {
																																			if (trim($_POST['OFECHA_VEN']) <> '') {
																																				$Fecha_Ven = $_POST['OFECHA_VEN'];
																																			} else {
																																				$Fecha_Ven = '0000/00/00';
																																			}
																																		}
