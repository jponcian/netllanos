...
<body style="background: transparent !important;">
	...
	if ($_POST['CMDANULAR'] == 'Anular') {
		if ($_POST['OMOTIVO'] <> '') {
			...
			while ($I <= $_SESSION['VARIABLE1']) {
				if (!empty($_POST[$I])) {
					$consulta = "UPDATE expedientes_rif SET Usuario=" . $_SESSION['CEDULA_USUARIO'] . ", Status = 9, FechaAnulacion = DATE(NOW()), MotivoAnulacion = '" . $_POST['OMOTIVO'] . "', Fecha_proceso = CURRENT_TIMESTAMP() WHERE id_expediente=" . $_POST[$I] . ";";
	...
	if ($_POST['OSEDE'] > 0) {
		// CONSULTA
		$consulta = "SELECT id_expediente, numero, anno, sector, rif, contribuyente, funcionario FROM vista_exp_rif WHERE sector=" . $_POST['OSEDE'] . " AND status=0 ORDER BY anno, numero;";
