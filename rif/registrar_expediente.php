...
if ($_POST['CMDGUARDAR'] == "Guardar") {
	if ($_POST['ORIF'] <> "" and $_POST['OSUPERVISOR'] > 1 and $_POST['OFISCAL'] > 1 and $_POST['OSEDE'] > 0) {
		...
		$consulta_x = 'SELECT Max(Numero)+1 AS Maximo FROM expedientes_rif WHERE Sector=0' . $_POST['OSEDE'] . ' AND Anno=' . date('Y') . ';';
		...
		$guardar = 0;
		$consulta_existe = "SELECT Rif, Numero, Anno, Status FROM expedientes_rif WHERE Rif='" . strtoupper($_POST['ORIF']) . "' and Anno=" . date("Y") . " and Status=0";
		$tabla_existe = mysql_query($consulta_existe);
		$cantidad = mysql_num_rows($tabla_existe);
		if ($cantidad < 1) {
			// GUARDADO DE LOS DATOS
			$consulta = "INSERT INTO expedientes_rif (Numero, Anno, Rif, FechaRegistro, Coordinador, Funcionario, Usuario, Sector, Status) VALUES (" . $Maximo . ", " . date("Y") . ", '" . strtoupper($_POST['ORIF']) . "', date(now()), " . $_POST['OSUPERVISOR'] . ", " . $_POST['OFISCAL'] . ", " . $_SESSION['CEDULA_USUARIO'] . ", " . $_POST['OSEDE'] . ",0);";
...
<body style="background: transparent !important;">
	...
	<div align="center">
		...
		if ($_SESSION['VARIABLE'] == 'REGISTRADA') {
			...
			$consulta_x = "SELECT anno, numero FROM expedientes_rif WHERE sector=" . $_POST['OSEDE'] . " ORDER BY anno DESC , numero DESC;";
			$tabla_x = mysql_query($consulta_x);
			if ($registro_x = mysql_fetch_object($tabla_x)) {
				...
				//------------------
				echo '<form name="form2" method="post" action="formatos/portada.php?num=' . $_SESSION['NUMERO'] . '&anno=' . $_SESSION['ANNO'] . '&sede=' . $_SESSION['SEDE'] . '" target="_blank">';
