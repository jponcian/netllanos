...
// ------------------------------------------------------------------------
$consulta_b = "SELECT a_banco.banco, a_banco.Descripcion FROM ce_pagos, a_banco, a_agencia WHERE a_agencia.id_agencia = ce_pagos.Agencia AND ce_pagos.Sector = ".$_SESSION['SEDE_USUARIO']." AND Month(ce_pagos.Fecha_Pago)=".$_SESSION['VARIABLE']." AND Year(ce_pagos.Fecha_Pago)=".$_SESSION['VARIABLE2']." AND a_banco.id_banco = a_agencia.id_banco GROUP BY a_agencia.id_banco ORDER BY a_agencia.id_banco ASC";
