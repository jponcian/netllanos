...
if ($_POST['CMDGUARDAR'] == "Guardar") {
	if ($_POST['OSEDE'] > 0 and $_POST['ORIF'] <> "" and $_POST['OPLANILLA'] <> "" and $_POST['OFECHA'] <> "" and $_POST['OMONTO'] <> "" and $_POST['OFECHAI'] <> "") {
		...
		if ($existe < 1) {
			...
			$consulta = "UPDATE ce_providencia SET rif='" . strtoupper($_POST['ORIF']) . "', id_tributo=" . $_POST['OTRIBUTO'] . ", planilla='" . $_POST['OPLANILLA'] . "', fecha_planilla='" . $_POST['OFECHA'] . "', monto_planilla=" . $_POST['OMONTO'] . ", inicio_sujeto_especial='" . $_POST['OFECHAI'] . "', usuario=" . $_SESSION['CEDULA_USUARIO'] . ", sector=" . $_POST['OSEDE'] . " WHERE id_providencia=" . $_POST['OID'] . ";";
...
if ($_POST['ONUMERO'] > 0 and $_POST['ONUMERO_ANTERIOR'] <> $_POST['ONUMERO']) {
	$consulta_x = 'SELECT id_providencia, numero, anno, rif, fecha_registro, id_tributo, planilla, fecha_planilla, monto_planilla, inicio_sujeto_especial FROM ce_providencia WHERE sector=0' . $_POST['OSEDE'] . ' AND anno=' . $_POST['OANNO'] . ' AND numero=' . $_POST['ONUMERO'] . ';';
	$tabla_x = mysql_query($consulta_x);
	$registro_x = mysql_fetch_object($tabla_x);
	//---------
	$_POST['ORIF'] = $registro_x['rif'];
	$_POST['OFECHA'] = voltea_fecha($registro_x['fecha_planilla']);
	$_POST['OTRIBUTO'] = $registro_x['id_tributo'];
	$_POST['OPLANILLA'] = $registro_x['planilla'];
	$_POST['OMONTO'] = $registro_x['monto_planilla'];
	$_POST['OFECHAI'] = $registro_x['inicio_sujeto_especial'];
	...
	<form name="form1" method="post" action="">
		<div align="center">
			<table width="50%" border="1" align="center">
				<tr>
					<td align="center" bgcolor="#FF0000" colspan="9"><span class="Estilo7"><u>Modificar Providencia Administrativa - Calificaci�n Sujeto Pasivo Especial</u></span></td>
				</tr>
				...
				<td><label>
						<div align="center"><span class="Estilo1">
								<select name="ONUMERO" size="1" onChange="this.form.submit()">
									...
									<?php
									if ($_POST['OANNO'] > 0) {
										$consulta_x = 'SELECT numero FROM vista_ce_providencias WHERE sector=' . $_SESSION['SEDE_USUARIO'] . ' AND anno=' . $_POST['OANNO'] . ' and motivo_anulacion is null ORDER BY numero DESC;';
										$tabla_x = mysql_query($consulta_x);
										if ($registro_x = mysql_fetch_object($tabla_x)) {
											echo '<option ';
				...
				<td><label>
						<div align="center"><span class="Estilo15">
								<input type="text" name="OFECHA" value="<?php echo $_POST['OFECHA']; ?>" size="8" readonly></span></div>
					</label></td>
			</table>
