				...
				<td colspan="4"><label>
						<select name="OTERMINAL" size="1" onChange="this.form.submit()">
							<option value="100"> ->-> TODOS <-<- </option>
									<?php
									$consulta_x = "SELECT RIGHT(ce_pagos.Rif,1) AS Terminal FROM ce_pagos WHERE ce_pagos.Sector = " . $_POST['OSEDE'] . " GROUP BY RIGHT(ce_pagos.Rif,1)";
									$tabla_x = mysql_query($consulta_x);
									while ($registro_x = mysql_fetch_object($tabla_x)) {
										echo '<option';
