...
$consulta = "SELECT * FROM resoluciones WHERE anno_expediente=".$_SESSION['ANNO_PRO']." AND num_expediente=".$_SESSION['NUM_PRO']." and id_origen=".$_SESSION['ORIGEN']." and id_sector=".$_SESSION['SEDE_USUARIO'];
...
if ($registro = mysql_fetch_object($tabla))
	...
	{
	// CONSULTA DEL MAXIMO
	$consulta = "SELECT Max(numero)+1 AS Maximo FROM resoluciones WHERE year(fecha)=year(date(now())) and id_origen=".$_SESSION['ORIGEN']." and id_sector=".$_SESSION['SEDE_USUARIO'];
	$tabla = mysql_query($consulta);
	...
	$consulta = "INSERT INTO resoluciones (id_sector, id_origen, anno_expediente, num_expediente, anno, numero, fecha) SELECT ".$_SESSION['SEDE_USUARIO'].", ".$_SESSION['ORIGEN'].", ".$_SESSION['ANNO_PRO'].", ".$_SESSION['NUM_PRO'].", year(date(now())), ".$Maximo.", date(now())";
	$tabla = mysql_query($consulta);
	}
// FIN
...
$registro = mysql_fetch_object($tabla);
// ---------------------
$anno_res = $registro->anno;
$num_res = $registro->numero;
$fecha_res = $registro->fecha;
// ---------------------

////////// INFORMACION DEL EXPEDIENTE
$consulta_datos = "SELECT * FROM vista_exp_especiales WHERE anno=0".$_SESSION['ANNO_PRO']." AND numero=0".$_SESSION['NUM_PRO']." AND sector =0".$_SESSION['SEDE_USUARIO'].";";
...
$resolucion = $SIGLAS.$_SESSION['ANNO_PRO'].'/'.sprintf("%004s", $_SESSION['NUM_PRO']) ."/". $anno_res .'/'. sprintf("%004s", $num_res);
$resolucion_prov = $SIGLAS.$_SESSION['ANNO_PRO'].'/'.sprintf("%004s", $_SESSION['NUM_PRO']);
// ---------------------
...
$pdf->SetFont('Times','B',13-$tamaño);
	...
	$pdf->Ln(5);
	switch ($_SESSION['SEDE_USUARIO']) {
    case 1:
        $estado = "Guárico";
		$conector1 = "de la ";
		$conector2 = "a la ";
		$ubicacion = "Gerencia Regional de Tributos Internos de la Región los Llanos"; 
        break;
    case 2:
        $estado = "Guárico";
		$conector1 = "del ";
		$conector2 = "al ";
		$ubicacion = "Sector de Tributos Internos San Juan de Los Morros de la Gerencia Regional de Tributos Internos de la Región los Llanos"; 
        break;
    case 3:
        $estado = "Apure";
		$conector1 = "del ";
		$conector2 = "al ";
		$ubicacion = "Sector de Tributos Internos San Fernando de Apure de la Gerencia Regional de Tributos Internos de la Región los Llanos"; 
        break;
    case 4:
        $estado = "Guárico";
		$conector1 = "de la ";
		$conector2 = "a la ";
		$ubicacion = "Unidad de Tributos Internos Altagracia de Orituco de la Gerencia Regional de Tributos Internos de la Región los Llanos"; 
        break;
    case 1:
        $estado = "Guárico";
		$conector1 = "del ";
		$conector2 = "al ";
		$ubicacion = "Sector de Tributos Internos Valle de la Pascua de la Gerencia Regional de Tributos Internos de la Región los Llanos"; 
        break;
}
	$txt='En '.$registro_datos->nombre.', Estado '.$estado.', sede '.$conector1.$ubicacion.', del Servicio Nacional Integrado de Administración Aduanera y Tributaria (SENIAT); de conformidad con lo establecido en los artículos 89, 131 numeral 2, 137 numeral 2 y 182 del Código Orgánico Tributario publicado en Gaceta Oficial N° 6.152 Extraordinario del 18 de noviembre de 2014 , en lo sucesivo COT, la División de Sujetos Pasivos Especiales adscrita '.$conector2.$ubicacion.', del Servicio Nacional Integrado de Administración Aduanera y Tributaria (SENIAT), en uso de las facultades legales conferidas en el artículo 102, numeral 7, en concordancia con el numeral 18 de la Resolución Nº 32 sobre la Organización, Atribuciones y Funciones del SENIAT, publicada en Gaceta Oficial Nº 4.881 Extraordinario, de fecha 29/03/1995 y el artículo 4 numeral 9 de la Ley del SENIAT, publicada en Gaceta Oficial Nº 37.329 de fecha 08/11/2001, el articulo 2 de la resolución SNAT/2002/Nº 913, publicada en Gaceta Oficial 37.398, de fecha 06/02/2002 y el artículo 2 de la Providencia Administrativa SNAT/2009/0077 publicada en Gaceta Oficial Nº 39.246 de fecha 20/08/2009; procede a emitir la presente Resolución, en virtud de los hechos que se exponen a continuación:';
...
$consulta_xxx = "SELECT especial FROM vista_sanciones_aplicadas WHERE especial=3 and serie<>38 AND origen_liquidacion=0".$_SESSION['ORIGEN']." AND anno_expediente=".$_SESSION['ANNO_PRO']." AND num_expediente=".$_SESSION['NUM_PRO']." AND sector=".$_SESSION['SEDE_USUARIO'].";";
...
$consulta = "SELECT id_liquidacion FROM vista_sanciones_aplicadas WHERE serie<>29 AND serie<>38 AND origen_liquidacion=0".$_SESSION['ORIGEN']." AND anno_expediente=".$_SESSION['ANNO_PRO']." AND num_expediente=".$_SESSION['NUM_PRO']." AND sector=".$_SESSION['SEDE_USUARIO'].";";
$tabla = mysql_query($consulta);
...
$consulta = "SELECT * FROM vista_sanciones_aplicadas WHERE serie<>29 AND serie<>38 AND origen_liquidacion=0".$_SESSION['ORIGEN']." AND anno_expediente=".$_SESSION['ANNO_PRO']." AND num_expediente=".$_SESSION['NUM_PRO']." AND sector=".$_SESSION['SEDE_USUARIO'].";";
...
$consulta = "SELECT id_liquidacion FROM vista_sanciones_aplicadas WHERE concurrencia=2 and serie<>29 AND serie<>38 AND origen_liquidacion=0".$_SESSION['ORIGEN']." AND anno_expediente=".$_SESSION['ANNO_PRO']." AND num_expediente=".$_SESSION['NUM_PRO']." AND sector=".$_SESSION['SEDE_USUARIO'].";";
...
if ($registro_xxx = mysql_fetch_object($tabla_xxx))
	...
	while ($registro_r = mysql_fetch_object($tabla_r))
		...
		$consulta = "SELECT * FROM vista_sanciones_aplicadas WHERE id_sancion=".$registro_r->id_sancion." AND origen_liquidacion=0".$_SESSION['ORIGEN']." AND anno_expediente=".$_SESSION['ANNO_PRO']." AND num_expediente=".$_SESSION['NUM_PRO']." AND sector=".$_SESSION['SEDE_USUARIO']." ORDER BY monto_ut DESC, periodoinicio, periodofinal;";
...
$consulta = "SELECT id_liquidacion FROM vista_sanciones_aplicadas WHERE serie=38 and origen_liquidacion=0".$_SESSION['ORIGEN']." AND anno_expediente=".$_SESSION['ANNO_PRO']." AND num_expediente=".$_SESSION['NUM_PRO']." AND sector=".$_SESSION['SEDE_USUARIO'].";";
...
if ($registro_i = mysql_fetch_object($tabla_i))
	...
	$consulta = "SELECT * FROM vista_sanciones_aplicadas WHERE serie=38 and origen_liquidacion=0".$_SESSION['ORIGEN']." AND anno_expediente=".$_SESSION['ANNO_PRO']." AND num_expediente=".$_SESSION['NUM_PRO']." AND sector=".$_SESSION['SEDE_USUARIO']." ORDER BY id_tributo;";
...
$pdf->Cell(0,5,'C.I. N°:     __________________________________');
$pdf->Ln(5);
