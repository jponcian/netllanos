...
class PDF extends FPDF
	{
		function Header()
		{
			...
			$this->SetXY(90,16);
			$this->Cell(0,5,mb_convert_case($area.' de Recaudacion - Registro de Información Fiscal (RIF)', MB_CASE_UPPER, "ISO-8859-1"),0,1,'C');
			$this->SetXY(90,21);
		...
		$consulta = "SELECT  COUNT(DISTINCTROW numero) AS cantidad FROM vista_transferidas_rif_liquidacion WHERE id_resolucion<1 AND fecha_transferencia_a_liq BETWEEN '".$_SESSION['INICIO']."' AND '".$_SESSION['FIN']."' AND sector=".$_SESSION['SEDE'].";";
		$tabla = mysql_query($consulta);
		if ($providencia = mysql_fetch_object($tabla)) { $cant_prov = $providencia->cantidad; };

		//OBTENEMOS LA CANTIDAD DE PLANILLAS
		$consulta = "SELECT  COUNT(numero) AS cantidad FROM vista_transferidas_rif_liquidacion WHERE id_resolucion<1 AND fecha_transferencia_a_liq BETWEEN '".$_SESSION['INICIO']."' AND '".$_SESSION['FIN']."' AND sector=".$_SESSION['SEDE'].";";
		$tabla = mysql_query($consulta);
...
$linea = 0;
$i=1;
$consulta = "SELECT anno, numero, fecha_emision, rif, contribuyente, serie, descripcion, monto, fecha_transferencia_a_liq, sector FROM vista_transferidas_rif_liquidacion WHERE id_resolucion<1 AND fecha_transferencia_a_liq BETWEEN '".$_SESSION['INICIO']."' AND '".$_SESSION['FIN']."' AND sector=".$_SESSION['SEDE']." ORDER BY anno, numero ASC;";
$tabla = mysql_query($consulta);
