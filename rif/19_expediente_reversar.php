...
if ($_POST['CMDCONCLUIR'] == "Reversar") {
	...
	$consulta = "UPDATE expedientes_rif SET Status=0, fecha_conclusion = '', Usuario=" . $_SESSION['CEDULA_USUARIO'] . " WHERE anno=" . $_SESSION['ANNO_PRO'] . " AND numero=" . $_SESSION['NUM_PRO'] . " AND Sector=" . $_SESSION['SEDE_USUARIO'] . ";";
					...
					<div align="center"><span class="Estilo1">
							<select name="OSEDE" size="1" onChange="this.form.submit()">
								...
								if ($_SESSION['ADMINISTRADOR'] > 0) {
									$consulta_x = 'SELECT sector as id_sector, nombre FROM vista_exp_rif WHERE status=4 GROUP BY sector;';
									...
									$consulta_x = 'SELECT sector as id_sector, nombre FROM vista_exp_rif WHERE status=4 and sector=' . $_SESSION['SEDE_USUARIO'] . ' GROUP BY sector;';
									$tabla_x = mysql_query($consulta_x);
									while ($registro_x = mysql_fetch_array($tabla_x)) {
										echo '<option ';
										if ($_POST['OSEDE'] == $registro_x['id_sector']) {
				...
				<td width="36%"><label>
						<div align="center"><span class="Estilo1">
								<select name="ONUMERO" size="1" onChange="this.form.submit()">
									...
									<?php
									if ($_POST['OANNO'] > 0) {
										$consulta_x = 'SELECT numero FROM vista_exp_rif WHERE status=4 AND anno=' . $_POST['OANNO'] . ' AND sector=0' . $_POST['OSEDE'] . ';';
										$tabla_x = mysql_query($consulta_x);
										while ($registro_x = mysql_fetch_array($tabla_x)) {
											echo '<option ';
