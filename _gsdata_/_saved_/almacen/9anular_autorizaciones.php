...
<body style="background: transparent !important;">
	...
	<div class="mx-auto d-block" style="width:85%;max-width:1200px;">
		<div class="card border-danger p-0 m-0">
			...
			<div class="card-body p-2">
				...
				<div id="div1">
					<?php include "9tabla_autorizacion.php"; ?>
				</div>
			</div>
		</div>
	</div>
	...
	<div id="aprobarModalCustom">
		<div id="aprobarModalContent">
			<div id="aprobarModalHeader">
				<span id="aprobarModalTitle">Anular Autorización</span>
	...
	<script>
		...
		function ver() {
			...
			$('#div1').load('9tabla_autorizacion.php?division=' + division + '&fecha=' + fecha, function () {
		...
		function showAprobarModal(url) {
			...
			fetch(url)
				...
				.then(html => {
					body.innerHTML = html;
					// Asignar evento al botón Guardar del modal (ahora por clase y texto)
					const btnGuardar = Array.from(body.querySelectorAll('button.btn-danger')).find(btn => btn.textContent.includes('Anular'));
					const form = body.querySelector('form');
					if (btnGuardar && form) {
						btnGuardar.onclick = function (e) {
							e.preventDefault();
							var notas = form.querySelector('#ONOTAS');
							if (!notas.value.trim()) {
								notas.classList.add('is-invalid');
								notas.focus();
								...
								notas.classList.remove('is-invalid');
							}
							// Guardar vía AJAX
							...
							fetch('9anular_autorizacion_guardar.php?solicitud=' + encodeURIComponent(solicitud), {
								method: 'POST',
								body: formData
							})
								.then(r => r.text())
								.then(resp => {
									// Si la respuesta contiene éxito, cerrar modal y mostrar mensaje
									if (/Procesada\s*Exitosamente|anulada\s*exitosamente|guardado\s*con\s*éxito/i.test(resp)) {
										...
										Swal.fire({
											...
											text: 'La autorización fue anulada exitosamente.',
