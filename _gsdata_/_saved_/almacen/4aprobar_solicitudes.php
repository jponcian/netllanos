		...
		function showAprobarModal(url) {
			...
			fetch(url)
				...
				.then(html => {
					...
					const btnGuardar = Array.from(body.querySelectorAll('button.btn-danger')).find(btn => btn.textContent.includes('Guardar'));
					...
					if (btnGuardar && form) {
						btnGuardar.onclick = function (e) {
							...
							fetch('4aprobar_solicitud_guardar.php?solicitud=' + encodeURIComponent(solicitud), {
								method: 'POST',
								body: formData
							})
								.then(r => r.text())
								.then(resp => {
									...
									if (/Procesada\s*Exitosamente|aprobada\s*exitosamente|guardado\s*con\s*éxito/i.test(resp)) {
										modal.classList.remove('show');
										ver();
										Swal.fire({
											icon: 'success',
											title: 'Solicitud aprobada',
											text: 'La solicitud fue procesada exitosamente.',
