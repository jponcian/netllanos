	...
	<script>
		...
		function showAnularModal(url) {
			...
			fetch(url)
				...
				.then(html => {
					body.innerHTML = html;
					// Ejecutar scripts embebidos (similar a flujo de aprobar) para que se inicialice la validación interna
					try {
						const scripts = body.querySelectorAll('script');
						scripts.forEach(sc => {
							// Evitar múltiples inicializaciones de la misma lógica
							if (window.__ANULAR_VALIDACION_INIT && !sc.src && sc.textContent.includes('initValidacionAnular')) {
							...
							if (sc.src) {
								...
								nuevo.textContent = sc.textContent;
							}
							document.head.appendChild(nuevo);
						});
						window.__ANULAR_VALIDACION_INIT = true;
					} catch (e) { console.warn('No se pudieron ejecutar scripts del modal anular:', e); }
