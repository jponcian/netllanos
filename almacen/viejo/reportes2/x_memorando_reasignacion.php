...
if ($registro = mysql_fetch_object($tabla))
	...
	$txt = $registro->siglas.$registro->anno.'/'.$registro->numero;
	$pdf->Cell(0,5,$txt);
	$pdf->Ln(8);
	...
	if ($registro->id_division_destino<>9)
		{	
		// POR SI HAY VARIOS BIENES
		if ($i==1) 
			{
			$txt='Tengo el agrado de dirigirme a usted, en la oportunidad de reasignarle, el bien nacional descrito en el comprobante anexo, el cual quedará adscrito al inventario bajo su responsabilidad por lo que se requiere tomar en cuenta las Normas Básicas Generales de Control Interno de los Bienes Nacionales:';
			}
		else
			{
			$txt='Tengo el agrado de dirigirme a usted, en la oportunidad de reasignarle, los bienes nacionales descritos en el comprobante anexo, los cuales quedarán adscritos al inventario bajo su responsabilidad por lo que se requiere tomar en cuenta las Normas Básicas Generales de Control Interno de los Bienes Nacionales:';
			}
		...
		$txt='En este sentido se le solicita la máxima colaboración para dar cumplimiento a la Normativa legal vigente.';
		$pdf->MultiCell(0,5,$txt);
		$pdf->Ln(4); 
		}
	// POR SI ESTAN DEVOLVIENDO LOS BIENES
		...
		if ($i==1) 
			{
			$txt='Tengo el agrado de dirigirme a usted, en la oportunidad de brindarle un cordial y respetuoso saludo, y a su vez reasignarle, el bien nacional descrito en el comprobante anexo, que se encuentra asignado en esta División según nuestro inventario.';
			}
		else
			{
			$txt='Tengo el agrado de dirigirme a usted, en la oportunidad de brindarle un cordial y respetuoso saludo, y a su vez reasignarle, los bienes nacionales descritos en el comprobante anexo, que se encuentran asignados en esta División según nuestro inventario.';
			}
