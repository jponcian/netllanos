...
class PDF extends FPDF
{
	...
	$this->MultiCell(0,6,'INVENTARIO ACTUAL '.strtoupper($mes[(date("m")-1)]).' '.date(Y),$linea_muestra,'C',0);
	//---------------
...
$consulta = "SELECT timbre_inv_detallado.codigo, Sum(timbre_inv_detallado.cantidad) AS cantidad, timbre_inv.descripcion, timbre_inv.precio, timbre_inv.indice FROM timbre_inv_detallado INNER JOIN timbre_inv ON timbre_inv_detallado.codigo = timbre_inv.codigo GROUP BY timbre_inv_detallado.codigo, timbre_inv.descripcion, timbre_inv.precio, timbre_inv.indice ORDER BY timbre_inv.indice;";
...
while ($registro = mysql_fetch_object($tabla))
	...
	$consultax = "SELECT codigo, serial_desde, serial_hasta, cantidad FROM timbre_inv_detallado WHERE codigo='".$registro->codigo."' ORDER BY serial_desde;"; 
