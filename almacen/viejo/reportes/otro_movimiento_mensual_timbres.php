...
class PDF extends FPDF
{
	...
	$this->Cell($h,6,number_format(doubleval($cantidad0),2,',','.'),$linea_fija,0,'C',1);
	$this->Cell($j,6,number_format(doubleval($monto0),2,',','.'),$linea_fija,0,'C',1);
	
	$this->Cell($h,6,number_format(doubleval($cantidad1),2,',','.'),$linea_fija,0,'C',1);
	$this->Cell($j,6,number_format(doubleval($monto1),2,',','.'),$linea_fija,0,'C',1);
	
	$this->Cell($h,6,number_format(doubleval($cantidad0+$cantidad1),2,',','.'),$linea_fija,0,'C',1);
	$this->Cell($j,6,number_format(doubleval($monto0+$monto1),2,',','.'),$linea_fija,0,'C',1);
	
	$this->Cell($h,6,number_format(doubleval($cantidad2),2,',','.'),$linea_fija,0,'C',1);
	$this->Cell($j,6,number_format(doubleval($monto2),2,',','.'),$linea_fija,0,'C',1);
	
	$this->Cell($h,6,number_format(doubleval($cantidad0+$cantidad1-$cantidad2),2,',','.'),$linea_fija,0,'C',1);
...
list($mes,$anno)=explode('/',$_SESSION['var1']);
//--------------
$consulta = "SELECT timbre_inv.codigo, timbre_inv.precio, Sum(timbre_inv_detallado_mensual.cantidad) AS cantidad FROM timbre_inv_detallado_mensual INNER JOIN timbre_inv ON timbre_inv_detallado_mensual.codigo = timbre_inv.codigo GROUP BY timbre_inv.grupo, timbre_inv.codigo, timbre_inv.precio, timbre_inv.indice, timbre_inv_detallado_mensual.fecha HAVING timbre_inv.grupo='TIMBRES' AND (((timbre_inv_detallado_mensual.fecha)='".$_SESSION['var1']."')) ORDER BY timbre_inv.indice;";
...
while ($registro = mysql_fetch_object($tabla))
	...
	$pdf->SetFont('Times','',8);
	//------------------------
	$cantidad0 = $registro->cantidad;
	//------------------------

	//---INGRESOS DEL MES--------
	$consultax = "SELECT Sum(timbre_ingresos_detalle.cantidad) AS cantidad FROM timbre_ingresos INNER JOIN timbre_ingresos_detalle ON timbre_ingresos.numero = timbre_ingresos_detalle.numero_ingreso WHERE (((Month(fecha))=".$mes.") AND ((Year(fecha))=20".$anno.") AND ((timbre_ingresos_detalle.codigo)='".$registro->codigo."')) GROUP BY timbre_ingresos_detalle.codigo;";
	$tablax = mysql_query($consultax);
	if ($registrox = mysql_fetch_object($tablax))
		{		$cantidad1 = $registrox->cantidad;		}
	else
	...
	$pdf->Cell($b,6,number_format(doubleval($registro->precio),2,',','.'),$linea_fija,0,'C',0);
	//------------
	$pdf->Cell($h,6,number_format(doubleval($cantidad0),2,',','.'),$linea_fija,0,'R',0);
	$pdf->Cell($j,6,number_format(doubleval($cantidad0*$registro->precio),2,',','.'),$linea_fija,0,'R',0);
	//-------------------------
	$pdf->Cell($h,6,number_format(doubleval($cantidad1),2,',','.'),$linea_fija,0,'R',0);
	$pdf->Cell($j,6,number_format(doubleval($cantidad1*$registro->precio),2,',','.'),$linea_fija,0,'R',0);
	//-------------------------
	$pdf->Cell($h,6,number_format(doubleval($cantidad0 + $cantidad1),2,',','.'),$linea_fija,0,'R',0);
	$pdf->Cell($j,6,number_format(doubleval(($cantidad0 + $cantidad1) * $registro->precio),2,',','.'),$linea_fija,0,'R',0);
	//-------------------------
	$pdf->Cell($h,6,number_format(doubleval($cantidad2),2,',','.'),$linea_fija,0,'R',0);
	$pdf->Cell($j,6,number_format(doubleval($cantidad2 * $registro->precio),2,',','.'),$linea_fija,0,'R',0);
	//-------------------------
	$pdf->Cell($h,6,number_format(doubleval($cantidad0 + $cantidad1 - $cantidad2),2,',','.'),$linea_fija,0,'R',0);
	$pdf->Cell($j,6,number_format(doubleval(($cantidad0 + $cantidad1 - $cantidad2) * $registro->precio),2,',','.'),$linea_fija,0,'R',0);
	//----------------------
	$i++;
	$pdf->ln(6);
	//----------------------
