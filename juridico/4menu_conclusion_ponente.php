...
<head>
	...
	<title>Expedientes por Concluir</title>
					...
					<td height="27" align="center" bgcolor="#CCCCCC">
						<p class="Estilo7"><span class="Estilo1">
								<select name="OSEDE" size="1" onChange="this.form.submit()">
									...
									{
										$consulta_x = "SELECT expedientes_juridico.sector as sector, z_sectores.nombre as dependencia FROM expedientes_juridico, z_sectores WHERE z_sectores.id_sector = expedientes_juridico.sector and expedientes_juridico.status=2 GROUP BY expedientes_juridico.sector;";
									} else {
										// -------------------------------------
										$consulta_x = "SELECT expedientes_juridico.sector as sector, z_sectores.nombre as dependencia FROM expedientes_juridico, z_sectores WHERE z_sectores.id_sector = expedientes_juridico.sector AND expedientes_juridico.cedula_ponente = " . $_SESSION['CEDULA_USUARIO'] . " AND expedientes_juridico.sector = " . $_SESSION['SEDE_USUARIO'] . " AND expedientes_juridico.status=2 GROUP BY expedientes_juridico.sector;";
									}
									// -------------------------------------
		...
		<table class="formateada" width="55%" border=1 align=center>
			<tbody>
				...
				if ($_POST['OSEDE'] > 0) {
					if ($_SESSION['ADMINISTRADOR'] > 0) //or $_SESSION['SEDE_USUARIO']==1
					{
						$consulta_x = "SELECT expedientes_juridico.num_escrito_descargo, vista_contribuyentes_direccion.rif, vista_contribuyentes_direccion.contribuyente, expedientes_juridico.anno, expedientes_juridico.numero, z_sectores.nombre, expedientes_juridico.sector FROM expedientes_juridico INNER JOIN vista_contribuyentes_direccion ON vista_contribuyentes_direccion.rif = expedientes_juridico.rif INNER JOIN z_sectores ON z_sectores.id_sector = expedientes_juridico.sector WHERE expedientes_juridico.sector = " . $_POST['OSEDE'] . " AND expedientes_juridico.status = 2 ORDER BY rif ASC";
					} else {
						$consulta_x = "SELECT expedientes_juridico.num_escrito_descargo, vista_contribuyentes_direccion.rif, vista_contribuyentes_direccion.contribuyente, expedientes_juridico.anno, expedientes_juridico.numero, z_sectores.nombre, expedientes_juridico.sector FROM expedientes_juridico INNER JOIN vista_contribuyentes_direccion ON vista_contribuyentes_direccion.rif = expedientes_juridico.rif INNER JOIN z_sectores ON z_sectores.id_sector = expedientes_juridico.sector WHERE expedientes_juridico.sector = " . $_POST['OSEDE'] . " AND expedientes_juridico.cedula_ponente = " . $_SESSION['CEDULA_USUARIO'] . " AND expedientes_juridico.status = 1 ORDER BY rif ASC";
					}
					//-----------------
					...
					while ($registro = mysql_fetch_object($tabla)) {
						...
						echo '</div></td><td ><div align="center" class="Estilo5"><div align="center"><a href="4.1conclusion_ponente.php?rif=' . $registro->rif;
						echo '&num=';
						echo $registro->numero;
						echo '&anno=';
						echo $registro->anno;
