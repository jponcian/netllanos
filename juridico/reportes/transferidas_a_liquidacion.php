...
class PDF extends FPDF
	{
		function Header()
		{
			...
			$consulta_x = "SELECT nombre, tipo_division FROM z_sectores WHERE id_sector=".$_SESSION['SEDE'];
			$tabla_x = mysql_query($consulta_x);
			$regstro_x = mysql_fetch_object($tabla_x);
			...
			$this->SetXY(90,16);
			$this->Cell(0,5,mb_convert_case($area.' de Jurídico', MB_CASE_UPPER, "ISO-8859-1"),0,1,'C');
			$this->SetXY(90,21);
		...
		$consulta = "SELECT  COUNT(DISTINCTROW numero) AS cantidad FROM vista_expedientes_juridico WHERE fecha_transferida_liquidacion BETWEEN '".$_SESSION['INICIO']."' AND '".$_SESSION['FIN']."' AND sector=".$_SESSION['SEDE'].";";
		$tabla = mysql_query($consulta);
		if ($providencia = mysql_fetch_object($tabla)) { $cant_prov = $providencia->cantidad; };

		//OBTENEMOS LA CANTIDAD DE PLANILLAS
		$consulta = "SELECT  COUNT(numero) AS cantidad FROM vista_jur_detalle_planillas WHERE fecha_transferida_liquidacion BETWEEN '".$_SESSION['INICIO']."' AND '".$_SESSION['FIN']."' AND sector=".$_SESSION['SEDE'].";";
		$tabla = mysql_query($consulta);
...
$linea = 1;
$i=1;
$consulta = "SELECT anno, numero, fecha_gestion as fecha_emision, rif, contribuyente, serie, descripcion, monto, fecha_transferida_liquidacion as fecha_transferida_liquidacion, sector FROM vista_transferidas_jur_liquidacion WHERE fecha_transferida_liquidacion BETWEEN '".$_SESSION['INICIO']."' AND '".$_SESSION['FIN']."' AND sector=".$_SESSION['SEDE']." ORDER BY anno, numero ASC;";
$tabla = mysql_query($consulta); 
//echo $consulta;
...
while ($registro = mysql_fetch_object($tabla))
{
	...
	$y2=$pdf->GetY();
	$alto_contribuyente = ($y2-$y1);
	
	//-- FORMULA PARA CUADRAR LAS ALTURAS
	if ($alto_sancion > $alto_contribuyente) 	
		{	$con = $alto_sancion/($alto_contribuyente/5);	$san = 5;	$alto = $alto_sancion;	}	
	//------------------------------------
	if ($alto_sancion < $alto_contribuyente) 	
		{	$san = $alto_contribuyente/($alto_sancion/5);	$con = 5;	$alto = $alto_contribuyente;	}	
	//------------------------------------
	if ($alto_sancion == $alto_contribuyente) 	
		{	$san = 5;	$con = 5;	$alto = $alto_sancion;	}	
	//-----------------------------------
	...
	//---------
	$pdf->Cell($a,($alto),$registro->numero,$linea,0,'C'); 
	$pdf->Cell($b,($alto),voltea_fecha($registro->fecha_emision),$linea,0,'C'); 
	$pdf->Cell($c,($alto),$registro->rif,$linea,0,'C'); 
	//--- MULTICELL
	$x=$pdf->GetX();
	$pdf->MultiCell($d,($con),$registro->contribuyente,$linea,'J');
	//- PARA PONER LAS COORDENADAS DESPUES DEL MULTICELL
	$pdf->SetY($y1);
	$pdf->SetX($x+$d);
	//--------------------------------------------------
	$pdf->Cell($e,($alto),$registro->serie,$linea,0,'C',0); 
	//--- MULTICELL
	$x=$pdf->GetX();
	$pdf->MultiCell($f,$san,$registro->descripcion,$linea,'J');
	$y2=$pdf->GetY();
	//- PARA PONER LAS COORDENADAS DESPUES DEL MULTICELL
	$pdf->SetY($y1);
	$pdf->SetX($x+$f);
	//--------------------------------------------------
	$pdf->Cell($g,($alto),formato_moneda($registro->monto),$linea,0,'R'); 
	//--------------------
	
//	// ----- PARA EL CUADRO
//	$pdf->SetY($y1);
//	$pdf->Cell($a,($alto_sancion),"",1); 
//	$pdf->Cell($b,($alto_sancion),"",1); 
//	$pdf->Cell($c,($alto_sancion),"",1); 
//	$pdf->Cell($d,($alto_sancion),"",1); 
//	$pdf->Cell($e,($alto_sancion),"",1); 
//	$pdf->Cell($f,($alto_sancion),"",1); 
//	$pdf->Cell($g,($alto_sancion),"",1); 
//	//---------------------
	$pdf->Ln($y2-$y1);
}
