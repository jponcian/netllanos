...
$consulta = "SELECT *, date_format(fecha,'%d/%m/%Y') as fechares FROM expedientes_fraccionamiento, vista_contribuyentes_direccion WHERE expedientes_fraccionamiento.rif= vista_contribuyentes_direccion.rif AND expedientes_fraccionamiento.rif='".$rif."' AND status=1;";
...
$sector = $registro->sector;
// ------
list($anno,$mes,$dia)=explode('-',$fecha2);
...
$pdf->Text($t,34,$ciudad.',  '.strftime('%d', strtotime(date('m/d/Y',$fecha2))).' de '.$mes[(strftime('%m', strtotime(date('m/d/Y',$fecha2)))-1)].' del '.strftime('%Y', strtotime(date('m/d/Y',$fecha2))));
...
// --------- DETALLES
	$consulta = "SELECT liquidacion.liquidacion as Numero_Liquidacion, (liquidacion.monto_bs / liquidacion.concurrencia * liquidacion.especial) AS MontoCifras, (liquidacion.monto_ut / liquidacion.concurrencia * liquidacion.especial) AS UTCifras, date_format(liquidacion.fecha_impresion, '%d/%m/%Y') as FechaLiquidacion, date_format(liquidacion.periodoinicio, '%d/%m/%Y') as periodoinicio, date_format(liquidacion.periodofinal, '%d/%m/%Y') as periodofinal FROM liquidacion INNER JOIN a_sancion ON a_sancion.id_sancion = liquidacion.id_sancion WHERE liquidacion.rif='".$rif."' AND liquidacion.status = 50  AND liquidacion.id_sancion <> 2500  AND liquidacion.id_sancion <> 2501  AND fraccionada=9999 AND a_sancion.serie <> 38  ORDER BY liquidacion.liquidacion ASC;";	
	$tabla = mysql_query($consulta);
