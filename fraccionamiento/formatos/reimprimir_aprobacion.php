...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		////////// DATOS LA RESOLUCION
		$RESOLUCION = $SIGLAS."AP/".$año."/".sprintf("%004s", $numero);
		// ---------------------
...
$consulta = "SELECT numero FROM expedientes_fraccionamiento WHERE rif='".$rif."' AND status>0;";
$tabla = mysql_query($consulta);
$registro = mysql_fetch_object($tabla);
// ------ 

// ------ OBTENER LA INFORMACION DEL CONTRIBUYENTE
$consulta = "SELECT *, date_format(fecha,'%Y/%m/%d') as fechares FROM expedientes_fraccionamiento, vista_contribuyentes_direccion WHERE expedientes_fraccionamiento.rif= vista_contribuyentes_direccion.rif AND expedientes_fraccionamiento.rif='".$rif."' AND status>0;";
$tabla = mysql_query($consulta);
$registro = mysql_fetch_object($tabla);
// ------
$numero = $registro->numero;
