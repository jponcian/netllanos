...
class PDF extends FPDF
	{
		function Header()
		{
			...
			$this->Cell(0,5,'Relación de Expedientes Transferidas a Cobro bajo la transferencia N°: '.$_GET['memo'],0,1,'C');
			$this->Ln(4);
		...
		function Footer()
		{
		//OBTENEMOS LA CANTIDAD DE PROVIDENCIAS O EXPEDIENTES
		$consulta = "SELECT COUNT(DISTINCTROW sector, origen_liquidacion, anno_expediente, num_expediente) AS cantidad FROM vista_liquidacion_planillas WHERE fecha_tranferencia_a_cob<>'' AND memo2 = ".$_GET['memo']." AND sector=".$_GET['sede'].";";
		$tabla = mysql_query($consulta);
		if ($providencia = mysql_fetch_object($tabla)) { $cant_prov = $providencia->cantidad; };

		//OBTENEMOS LA CANTIDAD DE PLANILLAS
		$consulta = "SELECT COUNT(num_expediente) AS cantidad FROM vista_liquidacion_planillas WHERE fecha_tranferencia_a_cob<>'' AND memo2 = ".$_GET['memo']." AND sector=".$_GET['sede'].";";
		$tabla = mysql_query($consulta);
...
$consulta = "SELECT rif, contribuyente, liquidacion, periodoinicio, periodofinal, fecha_liquidacion, siglas, (monto_bs/concurrencia*especial) as monto FROM vista_liquidacion_planillas WHERE fecha_tranferencia_a_cob<>'' AND memo2 = ".$_GET['memo']." AND sector=".$_GET['sede'].";"; 
$tabla = mysql_query($consulta);
...
while ($registro = mysql_fetch_object($tabla))
{
	...
	$y1=$pdf->GetY();
	//---------
	if ($y1 > 180) { $pdf->AddPage(); $pdf->Ln(5);	$y1=$pdf->GetY();	}
	//----------------------------------
