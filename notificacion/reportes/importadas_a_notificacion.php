...
//			$region = $regstro_x->nombre;
			...
			$consulta_x = "SELECT fecha_importacion_a_not as fecha FROM vista_liquidacion_planillas WHERE memo = ".$_GET['memo']." AND sector=".$_GET['sede'].";";
			$tabla_x = mysql_query($consulta_x);
			$regstro_x = mysql_fetch_object($tabla_x);
			...
			$this->Cell(0,5,'Relación de Expedientes Importados a Notificación bajo la Transferencia N°: '.$_GET['memo'].' el día '.voltea_fecha($fecha),0,1,'C');
			$this->Ln(4);
		...
		$consulta = "SELECT COUNT(DISTINCTROW sector, origen_liquidacion, anno_expediente, num_expediente) AS cantidad FROM vista_liquidacion_planillas WHERE fecha_importacion_a_not<>'' AND memo = ".$_GET['memo']." AND sector=".$_GET['sede'].";";
		$tabla = mysql_query($consulta);
		if ($providencia = mysql_fetch_object($tabla)) { $cant_prov = $providencia->cantidad; };

		//OBTENEMOS LA CANTIDAD DE PLANILLAS
		$consulta = "SELECT COUNT(num_expediente) AS cantidad FROM vista_liquidacion_planillas WHERE fecha_importacion_a_not<>'' AND memo = ".$_GET['memo']." AND sector=".$_GET['sede'].";";
		$tabla = mysql_query($consulta);
...
$consulta = "SELECT rif, contribuyente, liquidacion, periodoinicio, periodofinal, fecha_liquidacion, siglas, (monto_bs/concurrencia*especial) as monto FROM vista_liquidacion_planillas WHERE fecha_importacion_a_not<>'' AND memo = ".$_GET['memo']." AND sector=".$_GET['sede'].";"; 
$tabla = mysql_query($consulta);
